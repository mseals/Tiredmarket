...
PRESETS = {
    ...
    'mistral': {
        ...
        # the user saw was NOT a real Mistral daily limit. It is this
...
# ─── v4.14.6.38-mute-transient-404: consecutive-404 counter ──────────
# Per-(provider_name, model) ring of recent 404 timestamps. A 404
# raises the count for that key; a SUCCESS to the same key resets it.
# The provider_endpoint_not_found Teacher AI event fires only when the
# count reaches _404_CONSECUTIVE_THRESHOLD within
# _404_WINDOW_SECONDS — so isolated runtime 404s under rate-limit
# pressure (the dominant pattern on Google Gemini's free tier) become
# a muted log line, while a real model retirement (many consecutive
# 404s with no intervening successes) still surfaces.
#
# Pattern mirrors the consecutive_429s counter in tm_provider_health
# (3-consecutive threshold).
_404_CONSECUTIVE_THRESHOLD = 3
_404_WINDOW_SECONDS = 300.0  # 5 minutes
_404_COUNTER: dict = {}      # (provider_name, model) -> list[ts]
_404_LOCK = threading.Lock()


def _404_key(provider: dict, fallback_name: str = '') -> tuple:
    """Build the consecutive-404 counter key for a provider config.
    Falls back gracefully when fields are missing — an empty model
    name just bucket-everything-on-this-provider, which is the right
    safety behavior."""
    nm = (str(provider.get('name') or provider.get('preset')
                or fallback_name or '').strip()
          or 'unknown')
    mdl = str(provider.get('model') or '').strip()
    return (nm, mdl)


def _record_provider_call_success_404_reset(provider: dict) -> None:
    """v4.14.6.38: a successful call to (provider, model) clears any
    accumulated 404 history for that key — only CONSECUTIVE 404s
    between successes should accumulate toward the popup threshold."""
    try:
        key = _404_key(provider)
        with _404_LOCK:
            _404_COUNTER.pop(key, None)
    except Exception:
        pass


def _emit_404_after_threshold(tm_ic, provider: dict,
                                provider_name: str,
                                log_fn=None) -> None:
    """v4.14.6.38: classify a 404 from this (provider, model).
      * Append the current timestamp to the (provider, model) counter.
      * Drop timestamps older than _404_WINDOW_SECONDS.
      * If the in-window count is below threshold, log a muted
        "transient 404 (n/THRESHOLD)" line via the existing log_fn
        (or via the registered app's _log if log_fn is None) and
        return WITHOUT emitting the Teacher AI event.
      * If the count reaches the threshold, emit the
        provider_endpoint_not_found event (a genuine persistent
        problem worth surfacing) AND reset the counter so the
        next series accumulates fresh.

    Any failure inside this helper is swallowed — the worst case is
    that the event fires more (or less) often than ideal; it can
    never break the AI dispatch path.
    """
    try:
        import time as _t
        now = _t.time()
        key = _404_key(provider, fallback_name=provider_name)
        model_name = key[1] or '<unspecified-model>'

        with _404_LOCK:
            lst = _404_COUNTER.setdefault(key, [])
            # Drop entries that fell out of the rolling window.
            cutoff = now - _404_WINDOW_SECONDS
            while lst and lst[0] < cutoff:
                lst.pop(0)
            lst.append(now)
            current = len(lst)

        if current >= _404_CONSECUTIVE_THRESHOLD:
            # Persistent problem — fire the event as before, then
            # reset so the next series starts at 0/threshold.
            try:
                tm_ic.emit_system_event(
                    'provider_endpoint_not_found',
                    context={'provider': provider_name,
                             'model': model_name,
                             'consecutive_404s': current,
                             'threshold': _404_CONSECUTIVE_THRESHOLD,
                             'window_seconds': _404_WINDOW_SECONDS})
            finally:
                try:
                    with _404_LOCK:
                        _404_COUNTER.pop(key, None)
        ...
        # Below threshold — muted log line only. Prefer the call-site
        # log_fn if available; fall back to the registered app's _log.
        try:
            _logf = log_fn if callable(log_fn) else None
            if _logf is None:
                _app = getattr(tm_ic, '_registered_app', None)
                _logf = getattr(_app, '_log', None) if _app else None
            if callable(_logf):
                _logf(
                    f"[router] {provider_name}/{model_name}: "
                    f"transient 404 ({current}/"
                    f"{_404_CONSECUTIVE_THRESHOLD}) — recovered via "
                    f"router failover; no action needed.", 'muted')
        except Exception:
            pass
    except Exception:
        # Helper must never raise into the dispatch path.
        pass
...
def call_provider(provider: dict, prompt: str,
    ...
    try:
        result = fn(provider, prompt, timeout)
        ...
        try:
            ...
            #
            # v4.14.6.38-mute-transient-404: a single isolated 404
            # under heavy rate-limit pressure (the 49/68 Gemini-429s-
            # per-hour pattern that ships with free-tier accounts)
            # was firing this "endpoint not found" event ~3 times per
            # 30 hours — surfaced as a "model gone forever" popup
            # even though the router failed over cleanly and consensus
            # completed. Mirror the classify_429 consecutive-counting
            # in tm_provider_health (3-consecutive threshold) and key
            # by (provider, model): only fire the event after
            # _404_CONSECUTIVE_THRESHOLD (3) 404s from the SAME
            # (provider, model) within _404_WINDOW_SECONDS (300 = 5
            # min). Below the threshold, log a muted "transient 404"
            # line so the failover is visible without alarming the
            # user. A successful call to the same (provider, model)
            # resets the counter (see _record_provider_call_success_
            # 404_reset above), so only TRULY CONSECUTIVE 404s
            # accumulate. A real model retirement (which produces
            # many consecutive 404s, not isolated ones) still trips
            # the threshold and surfaces the event.
            elif '404' in msg or 'not found' in msg:
                _emit_404_after_threshold(
                    _tm_ic, provider, provider_name,
                    log_fn=log_fn)
            ...
            pass
        raise
    # v4.14.6.38-mute-transient-404: the call succeeded. Reset the
    # per-(provider, model) 404 counter so the consecutive-404 popup
    # gate only fires when 404s ACCUMULATE between successes — i.e.
    # when the failure is persistent, not transient.
    try:
        _record_provider_call_success_404_reset(provider)
    except Exception:
        pass
    return result
...
def migrate_provider_names_v4140(cfg=None, data_dir=None) -> bool:
    ...
    try:
        ...
        if data_dir is None:
            candidates = [__import__('tm_paths').get_data_dir(), _P('data')]
            try:
...
def load_enabled_providers(data_dir=None) -> list:
    ...
    try:
        ...
        if data_dir is None:
            ...
            candidates = [__import__('tm_paths').get_data_dir(), _P44('data')]
            try:
...
def run_apis_for_scan_prediction(prompt: str, ticker: str, path: str,
    ...
    # v4.14.6.52-cerebras-context-guard: thread the prompt size into
    # eligibility so Cerebras is skipped (with backfill substitution)
    # on prompts that would exceed its ~8K free-tier context cap.
