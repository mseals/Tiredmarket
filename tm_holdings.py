...
# v4.14.6.49-depersonalize: resolve a user-name for AI prompts from cfg.
# Public ships with no `user_name` key → prompts read "the user" (fully
# anonymous). the user's main can set "user_name": "the user" in data/config.json
# to restore the personalized prompts. Robust against missing/None/wrong-
# type cfg; never raises.
def _user_name(cfg) -> str:
    try:
...
# the user saw as old-key lines in the activity log. The aliases are now
...
def compute_path_track_stats(predictions_path=None,
    ...
    if predictions_path is None:
        predictions_path = __import__('tm_paths').get_data_dir() / "predictions.jsonl"
...
class PromptBuilder:
    ...
    def __init__(self, cache: DataCacheLayer,
        ...
        # v4.14.6.49-depersonalize: cfg ref used only to resolve user_name
        # for the AI prompt strings (see _user_name). Optional; None or any
        # non-dict gracefully degrades to "the user".
        self.cfg = cfg
    ...
    def build_holding_analysis(self, holding: dict, path_key: str) -> tuple[str, dict]:
        ...
        # v4.14.6.49-depersonalize: user_name from cfg → "the user" default.
        _un = _user_name(self.cfg)

        lines = []
        lines.append(f"You are analyzing a position for {_un}. Be direct, commit "
        ...
        lines.append(f"  Given {_un}'s chosen path ({path_info['name']}) and "
    ...
    def build_locked_analysis(self, holding: dict, path_key: str) -> tuple[str, dict]:
        ...
        lines = []
        lines.append(f"You are watching a LOCKED position for {_un}. They cannot "
...
class HoldingsWindow:
    ...
    def _build_candidate_prompt(self, ticker, notes, path,
        ...
        # drainfix-2026-06-15 (v4.14.6.40-candidate-prompt-soften):
        # mirror the May-2026 Tier-2 thesis-validation softening
        # (v4.14.5.14-layer2-thesis-validation, tm_consensus.py:613-694).
        # When THIS ticker already has an open Layer-1 BUY for THIS path,
        # the candidate scan must SECOND-OPINION that thesis, not ask
        # "would you BUY this from scratch?" — the latter is the exact
        # framing that the pre-May Tier-2 prompt used to produce a
        # 95% CONTRADICTED rate (documented at tm_consensus.py:613-621).
        # The candidate prompt was MISSED by that softening pass and is
        # the root cause of the overnight board drain: every algo-emit
        # BUY (3+ days past maturity guard, no longer recency-skipped
        # before drainfix Part B) gets re-scanned with the old
        # "from-scratch" framing, the AI returns AVOID on extended
        # technicals (per the explicit hardening below), and
        # tm_discover.OUTCOME_CONTRADICTED marks the BUY contradicted →
        # board drops it on the next refresh_recommend_cache tick.
        #
        # Lookup is read-only and tolerant: tm_consensus._layer1_thesis_for
        # returns None on any failure (no log, no predictions log, no
        # open BUY, wrong direction, closed status). Single-path mode
        # only — unified mode evaluates N paths so "the" thesis isn't
        # well-defined; that mode keeps the from-scratch framing.
        _open_l1_thesis = None
        if not _unified and self.predictions_log is not None:
            try:
                import tm_consensus as _tc_l1
                _open_l1_thesis = _tc_l1._layer1_thesis_for(
                    ticker, path, self.predictions_log)
            except Exception:
                _open_l1_thesis = None
        ...
        if _unified:
            lines = [
                ...
                f"position for {_un} (they do not own it). Evaluate it "
            ...
            # drainfix-2026-06-15: thesis-validation framing when an
            # open Layer-1 BUY exists; otherwise fresh-discovery framing.
            if _open_l1_thesis is not None:
                _l1_entry = (_open_l1_thesis.get('entry')
                             or _open_l1_thesis.get('buy_zone')
                             or (
                                 f"${_open_l1_thesis.get('buy_zone_low')}"
                                 f" - ${_open_l1_thesis.get('buy_zone_high')}"
                                 if (_open_l1_thesis.get('buy_zone_low')
                                     is not None and
                                     _open_l1_thesis.get('buy_zone_high')
                                     is not None) else '?'))
                _l1_target = _open_l1_thesis.get('target') or '?'
                _l1_stop = (_open_l1_thesis.get('stop')
                            or _open_l1_thesis.get('stop_loss') or '?')
                _l1_tf = _open_l1_thesis.get('timeframe_days') or '?'
                _l1_ts = (_open_l1_thesis.get('timestamp') or '')[:10]
                lines = [
                    f"A first-look analyzer (Layer 1) has already "
                    f"identified {ticker} as a BUY on path "
                    f"{path_info['name']}"
                    f"{' on ' + _l1_ts if _l1_ts else ''}. You are NOT "
                    f"being asked whether you would initiate this "
                    f"position from scratch right now — you are "
                    f"second-opinioning that specific thesis with the "
                    f"current data.",
                    "",
                    "LAYER 1 THESIS:",
                    f"  Entry: {_l1_entry}",
                ...
                lines = [
                    f"You are evaluating {ticker} as a potential new "
                    f"position for "
                    f"{_un}. They don't currently own this — should they "
                    f"consider entering?",
                    "",
                    f"PATH/GOAL: {path_info['name']} — "
                    f"{path_info.get('description', '')}",
        ...
        if notes:
            lines.append(f"USER NOTE ON THIS TICKER: {notes}")
            lines.append("")
        ...
        lines.append("QUESTION:")
        # drainfix-2026-06-15 (v4.14.6.40): branch the QUESTION text on
        # whether an open Layer-1 BUY exists for this (ticker, path).
        # Thesis-validation framing mirrors the proven Tier-2 softening
        # at tm_consensus.py:444-488. Fresh-discovery framing keeps the
        # original "should he enter?" question — but ALSO gets the
        # NEUTRAL/insufficient-data hatch and the news-optional guardrail
        # (both apply universally — newslessness must never count as
        # negative evidence for a keyless-news small-cap user).
        if _open_l1_thesis is not None:
            lines.append(
                f"  Second-opinion the Layer 1 BUY thesis on {ticker} "
                f"shown above. You are NOT being asked whether you'd "
                f"initiate this position from scratch. Decide:")
            lines.append(
                "    - SUPPORT (answer BUY): Layer 1's thesis is "
                "defensible. The entry/target/stop framework is "
                "reasonable given current data; you would not actively "
                "oppose a user acting on this call. SUPPORT does NOT "
                "mean \"I would buy this from scratch\" — it means "
                "\"this thesis holds up to scrutiny.\"")
            lines.append(
                "    - NEUTRAL (answer WATCH): the thesis is plausible "
                "but evidence is mixed. User should wait or watch.")
            lines.append(
                "    - OPPOSE (answer AVOID): the thesis is "
                "structurally broken — entry significantly off, target "
                "implausible, stop math doesn't work, OR a recent "
                "development (earnings miss, sector rotation, "
                "regulatory issue) invalidates the original setup. "
                "Note: absence of news is NOT such a development.")
            lines.append(
                "  Give a specific entry zone and exit levels in your "
                "answer (carry Layer 1's unless you'd revise them).")
        else:
            # v4.14.2 stage 4: vocabulary fix. Pre-stage-4 this asked
            # for BUY / HOLD / AVOID; HOLD is meaningless on tickers
            # the user doesn't own. WATCH replaces HOLD with the right
            # semantics: "interesting, wait for a better entry / clearer
            # signal."
            lines.append(
                f"  Should {_un} enter a position in {ticker}? "
                f"Consider their path, current technicals, and news. "
        ...
        # drainfix-2026-06-15: NEUTRAL/insufficient-data escape hatch.
        # Mirrors tm_consensus.py:482-484. Pre-drainfix the candidate
        # prompt forced a BUY/WATCH/AVOID commit even on thin data,
        # which biased toward AVOID for any cold-cache or thinly-
        # covered name. Now the model is explicitly told thin data is
        # WATCH, not AVOID.
        lines.append(
            "  If the data is genuinely insufficient to evaluate "
            "(thin history, cold cache, no recent news, no "
            "fundamentals), say WATCH with LOW confidence and explain "
            "what you'd need to see — do NOT default to AVOID on "
            "thin data.")
        # drainfix-2026-06-15: news-optional guardrail. The app is
        # 100% free and keyless news coverage is sparse for small-caps
        # (~73% of stored URLs are Google News redirect shells that
        # cannot be body-fetched; many tickers have no fetchable news
        # at all). Absent news must NOT count against the thesis —
        # the model has technicals + fundamentals + price levels.
        lines.append(
            "  News is supplementary context. If recent news is "
            "sparse or absent (common for thinly-traded names), do "
            "NOT treat that as evidence against the thesis — judge "
            "primarily on technicals, fundamentals, and the "
            "entry/target/stop math. Absent news is NEUTRAL, not "
            "negative.")
        # drainfix-2026-06-15: SOFTENED the v4.14.5.82-discovery-unlock
        # peak-safety hardening. The pre-drainfix text said "prefer
        # WATCH or AVOID over BUY" on extended technicals, which
        # auto-rejected the exact momentum setups the algo emits as
        # candidates (algo picks favour reclaiming/extended uptrends).
        # The narrower version: extension alone isn't disqualifying —
        # prefer WATCH over AVOID, and allow BUY when a strong
        # confirmed uptrend supports the thesis.
        lines.append(
            "  If extended technicals (high RSI, above the upper "
            "Bollinger band, far above the 20-day mean) are the ONLY "
            "concern and the rest of the thesis is supported, prefer "
            "WATCH over AVOID — but a strong, confirmed uptrend can "
            "still be a valid BUY. Don't reject an otherwise-supported "
            "thesis solely for being extended; reject only when "
            "extension combines with weak fundamentals, broken "
            "structure, or a specific bearish catalyst.")
