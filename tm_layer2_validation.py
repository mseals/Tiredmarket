...
# ── v4.14.6.68-consensus-capable-models ───────────────────────────────
# Back-compat: map legacy favorite DISPLAY names to canonical presets so a
# user config saved before the provider rename still matches. New configs use
# preset keys directly (groq/mistral/google), which match by_id via
# provider_canonical_id. Keyless: keyed by provider-TYPE, not any user's keys.
...
def _canon_pref_key(key):
    """Normalize a favorite provider key: lowercase + legacy-display-name →
    canonical preset. Never raises."""
    try:
        k = str(key or '').strip().lower()
        return _PROVIDER_KEY_ALIASES.get(k, k)
    except Exception:
        return key


def _validation_model_for(provider, preferred=None):
    """KEYLESS model resolver for tier-2 validation. Returns the model string
    to pin for `provider`:
      1. the cfg-preferred (capable) model IF the provider actually SERVES it
         (present in its models[]/model) — so we never pin an unresolvable
         string that would silently fall back to the small default;
      2. else the provider's SMARTEST served model (lowest capability_rank) —
         upgrades an unpinned backfill provider off its small default;
      3. else `preferred` (best effort) or None.
    Per provider-TYPE, never hardcoded to any user's providers. Never raises."""
    try:
        try:
            ...
            _cap = None
        served = []
        ms = provider.get('models') if isinstance(provider, dict) else None
        if isinstance(ms, (list, tuple)):
            served = [str(m).strip() for m in ms if str(m).strip()]
        cur = str((provider or {}).get('model') or '').strip()
        if cur and cur not in served:
            served = served + [cur]
        # 1. honor the cfg pin only if the provider serves it
        if preferred:
            pl = str(preferred).strip().lower()
            for m in served:
                if m.lower() == pl:
                    return m
        # 2. else upgrade to the smartest served model
        if served and _cap is not None:
            try:
                return min(served, key=lambda m: _cap.model_capability_rank(m))
            except Exception:
                pass
        # 3. best effort
        return preferred or (served[0] if served else None)
    except Exception:
        return preferred
    ...
    try:
        ...
        for entry in prefs:
            ...
            if bp in used_pids:
                continue
            used_pids.add(bp)
            ordered.append((base, _validation_model_for(base, model)))
        for p in avail:                       # other enabled providers
            bp = _pid(p)
            if bp in used_pids:
                continue
            used_pids.add(bp)
            # v4.14.6.68: pin the BACKFILL too — resolve to the provider's
            # capable model so a cooled-favorite substitution can't silently
            # weaken the panel with the provider's small default.
            ordered.append((p, _validation_model_for(p, None)))
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if not bool(cfg.get('use_layer2_validation', True)):
                ...
                continue
            # MODE-SWITCH (tier1-mode-2026-06-14): in 'algo_only' the
            # algo's pick is final — no AI consensus runs. The daemon
            # thread stays alive (so flipping the mode at runtime
            # re-engages it without restart) but sleeps the interval.
            # A once-per-process log line announces the dormant state
            # so the activity log doesn't go silent without explanation.
            # v4.14.6.74: resolve through _resolve_tier1_mode (NOT the raw
            # cfg key) so 'automatic' resolves correctly here too — automatic
            # + no AI → 'algo_only' (Tier-2 dormant); automatic + AI →
            # 'ai_plus_algo' (validation runs). Lazy import avoids a circular
            # import at module load. Fail-safe to the raw key on any error.
            try:
                import tm_queue_runner as _qr_mode
                _ms_mode = _qr_mode._resolve_tier1_mode(cfg)
            except Exception:
                try:
                    _ms_mode = str(cfg.get('tier1_mode') or '').strip().lower()
                except Exception:
                    _ms_mode = ''
            if _ms_mode == 'algo_only':
                if not getattr(_loop, '_mode_switch_dormant_logged', False):
                    try:
                        _log(app,
                             "[tier1-mode] algo_only: Tier-2 validation "
                             "off — algo picks are final.")
                    except Exception:
                        pass
                    try:
                        _loop._mode_switch_dormant_logged = True
                    except Exception:
                        pass
                stop_event.wait(interval)
                continue
