...
stock signal onto the user's existing position size:
  ...
  - Stock signal "BUY at this price" → for the user, that's BUY MORE
    (he should add to his existing position because the stock is still
  ...
  - Stock signal "SELL / overvalued / thesis broken" → for the user, that's
    SELL (full exit) or TRIM (partial exit if he wants to keep some
    exposure)
...
he already owns the stock; AVOID is meaningless here.
...
def build_owned_position_prompt(holding: dict, path_key: str,
    ...
    if q_idx == -1:
        ...
        # stock signal from the "the user already owns it" context, which
        ...
        new_question = (
            ...
            "he already holds 4 shares (or whatever the position info "
            "above says) at his cost basis. Your final DIRECTION should "
...
DECISION FRAMING (v4.14.5.14-layer2-thesis-validation, 2026-05-20):

You are NOT being asked whether you would initiate a position in this
stock from scratch right now. A first-look analyzer (Layer 1) has
already identified this as a BUY with the entry/target/stop shown
above. Your job is to second-opinion that specific thesis with the
current data:

  - SUPPORT: Layer 1's thesis is defensible. The entry/target/stop
    framework is reasonable given current data, the time horizon
    matches the path's intent, and you would not actively oppose a
    user acting on this call. SUPPORT does NOT mean "I would buy
    this from scratch" — it means "this thesis holds up to scrutiny."
  - NEUTRAL: The thesis is plausible but evidence is mixed. You can
    see why Layer 1 picked this, but you'd want confirmation (a
    catalyst, a clearer setup, a better entry) before committing.
    User should wait or watch.
  - OPPOSE: The thesis is structurally broken. Either the entry is
    significantly off, the target is implausible, the stop-loss
    math doesn't work, or there's a recent development (earnings
    miss, sector rotation, regulatory issue) that invalidates the
    original setup. The user should NOT act on this call as-is.

If the data is genuinely insufficient to second-opinion the thesis,
say NEUTRAL with LOW confidence and explain what you'd need to see.
...
class ConsensusRunner:
    ...
    def __init__(self,
        ...
        # explicit the user action still gates the pick.
    ...
    def _run(self) -> None:
        ...
        if self.providers and tm_apis is not None:
            if tm_router is not None:
                ...
                # Stash both for the iteration block below
    ...
    def _finalize(self) -> None:
        ...
        # the user action continues to write the gate-signal the dialog
