...
def answer_question(question: str,
    ...
    if model_answer_fn is None:
        def model_answer_fn(prompt, system=None):
            import tm_local_model as _lm
            return _lm.get_runtime().answer(prompt, system=system)

    try:
        import tm_local_model as _lm
        system = _lm.load_identity_system()
    except Exception:
        system = None
    try:
        text = model_answer_fn(d['prompt'], system=system)
    except Exception:
        # Model unavailable — fail to an honest non-answer, never a guess.
        text = (CANNED_APP_FACT_NO_CHUNK if d['action'] == 'answer_grounded'
                else "I can't answer that right now — try again in a moment.")
        return {'kind': d['kind'], 'action': d['action'], 'text': text,
                'uses_model': False, 'uses_cloud': False}
    return {'kind': d['kind'], 'action': d['action'],
            'text': (text or '').strip(),
            'uses_model': True, 'uses_cloud': False}
