...
def _is_eligible(profile: ProviderProfile, registry: Registry) -> bool:
    ...
    if not profile.is_usable():
        return False
    try:
        if registry.in_cooldown(profile.id):
            return False
    except Exception:
        pass
    # Legitimate DECLARED daily cap still enforced (e.g. GitHub Models ~50/day).
    # We no longer mis-learn an *observed* daily cap from a per-minute 429, so
    # effective_limit is the declared cap unless a real one was set elsewhere.
...
class Router:
    ...
    def fetch(self, data_type: str, return_status: bool = False, **kwargs):
        ...
        for profile in candidates:
            ...
            # Success!
            self._registry.record_success(profile.id)
            return _ret({
                'result': result,
                'source': profile.id,
    ...
    def _candidates(self, data_type: str, mode: str
        ...
        eligible = [p for p in mode_filtered
                     if _is_eligible(p, self._registry)]
        return eligible
    ...
    def _log_cooldown(self, profile, cd: dict) -> None:
