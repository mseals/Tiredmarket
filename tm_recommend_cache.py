...
def _ranking_score(cand, rec) -> float:
    """v4.14.6.69-display-validation-rank (Part 1): the score used to rank an
    open BUY into the displayed/bench tiers, with a fallback chain so an
    algo-emitted momentum BUY the momentum filter did NOT score is no longer
    sunk to 0.0 (the DNLI bug):
        momentum/filter score (if > 0)
        → algo_tier1 emit `algo_score`  (v69 numeric field)
        → score parsed from the emit raw_text ("Score NN.N / threshold")
          (covers pre-v69 records like the live DNLI pick)
        → coarse confidence proxy (HIGH/MODERATE/LOW — covers AI BUYs that
          carry no numeric score)
        → 0.0.
    Never raises."""
    try:
        s = float((cand or {}).get('score') or 0.0)
        if s > 0:
            return s
    except Exception:
        pass
    if isinstance(rec, dict):
        ...
        try:
            import re as _re
            m = _re.search(r'Score\s+(\d+(?:\.\d+)?)\s*/',
                           str(rec.get('raw_text') or ''))
            if m:
                return float(m.group(1))
        except Exception:
            pass
        conf = str(rec.get('confidence') or '').upper().strip()
        return {'HIGH': 75.0, 'MODERATE': 60.0, 'LOW': 45.0}.get(conf, 0.0)
    return 0.0


def _validation_tier(verdict) -> int:
    """v4.14.6.69-display-validation-rank (Part 2): primary sort key bucket so
    the displayed tier is validation-aware. Lower = promote sooner:
        VALIDATED_BUY                         → 0  (strong boost; surfaces)
        pending / SINGLE_VOICE / INCONCLUSIVE → 1  (fail-open MIDDLE — never
                                                    bench a fresh pick just
                                                    because the 180s daemon
                                                    hasn't validated it yet)
        MIXED                                 → 2
        CONTRADICTED                          → 3  (deprioritize; Layer-3
                                                    will drop it anyway)
    Never raises."""
    ...
    if v == 'MIXED':
        return 2
    return 1  # '', SINGLE_VOICE, INCONCLUSIVE, unknown → pending/middle
...
def refresh_recommend_cache(app, paths=None, force: bool = False) -> dict:
    ...
    for path in target_paths:
        ...
        if use_open_src:
            ...
            # v4.14.6.69-display-validation-rank (Part 2): pre-load the latest
            # Layer-2 verdict per ticker for THIS path so ranking can promote
            # VALIDATED_BUYs into the displayed tier (instead of ranking purely
            # on filter score, which benched 3/3-validated picks like DNLI).
            # Read-only; fail-open to {} (→ all pending → score-only ranking,
            # i.e. pre-v69 behavior) if the table is absent/unreadable.
            verdict_map = {}
            try:
                for _vr in conn.execute(
                        "SELECT ticker, verdict FROM "
                        "recommend_cache_validation WHERE path = ?", (path,)):
                    verdict_map[str(_vr[0]).upper()] = _vr[1]
            except Exception:
                verdict_map = {}
            ranked = []
            for tk, rec in ob:
                ...
                # Part 1: real score (algo emit score, not 0.0) for the sink fix.
                score = _ranking_score(cand, rec)
                # Part 2: validation tier is the PRIMARY sort key.
                vtier = _validation_tier(verdict_map.get(tk))
                if tk in prev:
                    fs = prev[tk][1]
                    try:
                        fs = float(fs)
                ...
                ranked.append((vtier, score, fs, tk, cand, rec))
            # (validation_tier ASC, score DESC, first_seen ASC, ticker ASC)
            ranked.sort(key=lambda x: (x[0], -x[1], x[2], x[3]))
            kept = [(tk, cand, rec)
                    for _vt, _s, _f, tk, cand, rec
