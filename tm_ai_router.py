...
def is_eligible(provider: dict, call_type: str,
    ...
    # FAILOVER-FIX (router-failover-2026-06-14): silently exclude
    # providers that are enabled-but-keyless so a new user's dead/keyless
    # provider is removed from the eligible set up front, instead of
    # burning one call per scan to discover the 401 and landing in cooldown.
    if not (provider.get('api_key') or '').strip():
        return (False, 'no api_key configured', None)
    ...
    # v4.14.6.51-rpm-gate-consensus: per-minute budget gate moved into
    # shared _minute_gate_check helper. Now called from BOTH
    # is_eligible AND is_eligible_for_model — pre-fix the gate was
    # only here, so consensus dispatch (model-aware path) skipped it
    # entirely and Gemini kept hitting its per-minute cap. Logic +
    # thresholds + log wording UNCHANGED.
    try:
        import tm_provider_learning as _tpl_rpm
        _fam = _tpl_rpm.provider_family(provider)
    except Exception:
        _fam = None
    if _fam:
        _model = resolve_provider_model(provider) or None
        _gate_res = _minute_gate_check(provider, _fam, _model, effective_cap)
        if _gate_res is not None:
            return _gate_res

    # v4.14.6.52-cerebras-context-guard: per-prompt context-window guard.
    # Same generic mechanism wired into is_eligible_for_model — mirror it
    # here so the legacy single-provider path (select_providers, backfill)
    # also skips Cerebras when the prompt would silently truncate.
    try:
        _guard_res = _context_guard_check(
            provider, _fam, estimated_prompt_chars)
        if _guard_res is not None:
            return _guard_res
    except Exception:
        pass
...
# v4.14.6.50-rpm-discovery: fallback ONLY when the provider does not
# send X-RateLimit-Limit-Requests-Minute headers. The numbers are
# documented family approximations (NOT contracts). When headers ARE
# present, tm_provider_learning.get_learned_minute_cap() overrides
# these — so a future Google quota cut auto-tracks via headers, not
# by editing this dict.
_RPM_SEED = {
    'mistral':   2,    # Mistral free: ~2 RPM rolling
    'gemini':    10,   # Gemini Flash-Lite free: 10-15 RPM (post-Dec 2025 cuts)
    'groq':      30,
    'cerebras':  30,
    'github':    15,
    'sambanova': 4,    # very low daily; rarely used
    'anthropic': 50,   # paid — money-burn protection only
    'openai':    60,   # paid
}
...
# provider family. None / missing = unlimited (no guard). Reusable for
# any future provider that silently truncates oversize prompts.
# Token budget leaves headroom for the model's response.
_PROVIDER_MAX_CONTEXT_TOKENS = {
    'cerebras': 7500,   # 8192 total - ~700 reserved for response
    # mistral, groq, gemini, anthropic, openai, github, sambanova: unlimited
    # (their free tiers are 32k+ — no need to guard)
}
# Conservative char->token heuristic: divide chars by this. Real English
# ratio is ~4 chars/token; we use 3.5 so we OVER-estimate (safer to skip
# a borderline prompt than send one that silently truncates).
_CHAR_PER_TOKEN_DIVISOR = 3.5


def _gate_log(provider, family, val, ceiling=None, source='?'):
    """v4.14.6.50-rpm-discovery: one-line concise telemetry when the
    per-minute gate proactively skips a provider. Reuses the existing
    router log channel; fail-silent if no app is registered."""
    try:
        name = (provider or {}).get('name') or (provider or {}).get('id') or '?'
        msg = (f"[rate-gate] {name} skipped: {val}"
               + (f"/{ceiling}" if ceiling else "")
               + f" RPM budget used ({source}; self-clears)")
        log = _resolve_router_log()
        if callable(log):
            try:
                log(msg, 'amber')
            except Exception:
...
def _minute_gate_check(provider: dict, fam: str, model,
                       effective_cap) -> Optional[tuple]:
    """v4.14.6.51-rpm-gate-consensus: shared per-minute budget gate.
    Extracted verbatim from is_eligible (v4.14.6.50 origin) so both
    is_eligible (legacy single-provider) AND is_eligible_for_model
    (model-aware consensus/lookup/recommend/scan) call the SAME logic.
    Pre-fix the gate lived only inside is_eligible -> all consensus
    dispatches skipped it -> Gemini kept hitting its per-minute cap.

    Returns:
      - (False, reason, effective_cap) if the budget gate skips
        this provider for this dispatch.
      - None if the gate passes (caller continues to its True return).
    Fail-open: any exception inside this helper -> returns None
    (treat as pass) so a gate fault never blocks a real call.
    """
    try:
        import tm_provider_learning as _tpl_rpm
        rpm_ceiling = None
        try:
            rpm_ceiling = _tpl_rpm.get_learned_minute_cap(
                fam, model=model)
            if rpm_ceiling is None:
                # Header-learned remaining tells us where we are in the
                # window if cap is unknown; that's still useful.
                rem = _tpl_rpm.get_learned_minute_remaining(
                    fam, model=model)
                if isinstance(rem, int) and rem <= _RPM_SAFETY:
                    _gate_log(provider, fam, rem,
                              source='header-remaining')
                    return (False,
                            f"per-minute budget exhausted (header "
                            f"reported {rem} remaining, self-clears)",
                            effective_cap)
        except Exception:
            rpm_ceiling = None
        if rpm_ceiling is None:
        ...
        if rpm_ceiling:
            try:
                ...
                used = 0
            if used >= max(1, rpm_ceiling - _RPM_SAFETY):
                _gate_log(provider, fam, used, ceiling=rpm_ceiling,
                          source='local-window')
                return (False,
                        f"per-minute budget used {used}/{rpm_ceiling} "
                        f"this minute (self-clears)",
                        effective_cap)
    except Exception:
        return None
    return None
...
def _context_guard_check(provider: dict, fam: str,
                          estimated_chars: Optional[int]
                          ) -> Optional[tuple]:
    """v4.14.6.52-cerebras-context-guard: per-provider context-window
    guard. Returns (False, reason, None) when this prompt would
    exceed the provider's hard context cap; None to pass.

    Fail-OPEN: any exception or missing estimate -> None (send the
    call). Cerebras silently TRUNCATES oversize prompts -> wrong
    consensus answers; this guard skips Cerebras for THIS prompt and
    lets the existing backfill substitute another provider into the
    slot. Generic / reusable: any provider family added to
    _PROVIDER_MAX_CONTEXT_TOKENS gets the same guard.
    """
    try:
        ...
        name = (provider or {}).get('name') or (provider or {}).get('id') or '?'
        msg = (f"[context-guard] {name} skipped: prompt ~{est_tokens} tok "
               f"> cap {cap_tokens} ({fam} silent-truncation guard; "
               f"substituting)")
        try:
            log = _resolve_router_log()
            if callable(log):
            ...
            pass
        return (False,
                f"prompt ~{est_tokens} tok exceeds {fam} context cap "
                f"{cap_tokens} (silent-truncation guard)",
                None)
    except Exception:
...
def _note_success_learning(provider_id: str,
    ...
    try:
        ...
        if _mdl:
            ...
            _tpl.note_success_headers(fam, meta, seed_cap=seed)
        # v4.14.6.50-rpm-discovery: stamp the sliding-minute counter so
        # the next is_eligible() check sees this dispatch. Functionally
        # equivalent to a pre-call stamp because dispatches are
        # sequential; both wind up counted in the trailing 60s window.
        try:
            _tpl.note_dispatch(fam, model=_mdl)
        except Exception:
            pass
...
def is_eligible_for_model(provider: dict, call_type: str,
    ...
    # v4.14.6.51-rpm-gate-consensus: per-minute budget gate now also
    # fires on the model-aware path (consensus / lookup / recommend /
    # scan). Pass canonical_model so per-model granularity is correct
    # — different canonical models served by the same provider have
    # different RPM windows (e.g. groq llama-3.1-8b vs llama-3.3-70b).
    try:
        import tm_provider_learning as _tpl_rpm
        _fam = _tpl_rpm.provider_family(provider)
    except Exception:
        _fam = None
    if _fam:
        _gate_res = _minute_gate_check(
            provider, _fam, canonical_model, effective_cap)
        if _gate_res is not None:
            return _gate_res

    # v4.14.6.52-cerebras-context-guard: per-prompt context-window guard.
    # Skips THIS provider for THIS prompt when estimated size exceeds the
    # provider's hard cap — existing substitution backfills the slot.
    try:
        _guard_res = _context_guard_check(
            provider, _fam, estimated_prompt_chars)
        if _guard_res is not None:
