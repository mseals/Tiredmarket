...
# ─── v4.14.6.42-ui-db-lock-fix: fill-state cache ──────────────────────
# The Tk main thread used to call _fill_mode_has_pending_work() every 30s
# from _tick_clock -> _update_freshness_label to drive a cosmetic
# "Filling/Idle" corner badge. That call acquired app.db.lock and ran
# four SELECTs (one per fill path). Under heavy background daemon load
# (queue runner, layer2/3, news/fundfile refresh, auto-refresh) the lock
# was held by daemons across slow Yahoo SSL handshakes (5-15s). Main
# thread blocked on the lock -> Windows judged the GUI frozen ("Not
# Responding"). A py-spy stack dump of the FROZEN process caught the main
# thread blocked at _db_lock inside _path_actionable_displayed inside
# _fill_mode_has_pending_work inside _update_freshness_label inside
# _tick_clock. That's the bug.
#
# Fix: keep an in-memory cached bool of the latest pending-work answer,
# guarded by a private lock that is NEVER held during DB I/O. The
# background threads (queue runner loop already calls this every poll
# tick) write the cache as a side effect of computing the real answer
# under _db_lock. The main thread reads the cache (private lock only,
# no _db_lock) via has_pending_work_cached(). Cosmetic badge can be a
# few seconds stale; that's fine.
_fill_state_cache_lock = threading.Lock()
_fill_state_cache = {'has_pending_work': False, 'ts': 0.0}
_FILL_STATE_CACHE_TTL_SECONDS = 5.0


def _fill_state_refresher(app) -> None:
    """v4.14.6.42-ui-db-lock-fix: background daemon that refreshes
    _fill_state_cache every ~5s by calling _fill_mode_has_pending_work
    (which populates the cache as a side effect). Belt-and-suspenders
    behind the queue-runner's per-tick call: keeps the cosmetic Filling/
    Idle badge truthful even if the runner is briefly idle or paused.
    Stops when app._queue_runner_stop fires. Never raises."""
    stop_event = getattr(app, '_queue_runner_stop', None)
    # Tiny startup grace so we don't race the runner-loop's own grace.
    try:
    ...
    while True:
        try:
            _fill_mode_has_pending_work(app)
        except Exception:
            pass
        try:
            if stop_event is not None:
                if stop_event.wait(_FILL_STATE_CACHE_TTL_SECONDS):
                    return
            else:
                time.sleep(_FILL_STATE_CACHE_TTL_SECONDS)
        except Exception:
            return


def has_pending_work_cached(app) -> bool:
    """v4.14.6.42-ui-db-lock-fix: lock-free (no app.db.lock) cached read of
    the fill-mode pending-work bool used by the corner-badge label. Safe to
    call from the Tk main thread; never touches the SQLite connection or its
    lock. Even if the cache is stale we return the cached value rather than
    compute on this thread — the main thread MUST NOT block on DB.

    The cache is refreshed as a side effect of background-thread calls to
    _fill_mode_has_pending_work (the queue-runner loop already calls that
    once per poll tick), and by the dedicated _fill_state_refresher daemon
    started from _runner_loop. Fail-OPEN to False on any error.
    """
    try:
        _fill_state_cache_lock.acquire()
        try:
            return bool(_fill_state_cache.get('has_pending_work', False))
        finally:
            _fill_state_cache_lock.release()
    except Exception:
        return False
...
def _runner_loop(app) -> None:
    ...
    # v4.14.6.42-ui-db-lock-fix: start the dedicated fill-state cache
    # refresher daemon. Belt-and-suspenders behind has_pending_work_cached
    # — the runner_loop already calls _fill_mode_has_pending_work each tick
    # (line ~360) which populates the cache as a side effect, but if the
    # runner ever blocks for an extended period the refresher keeps the
    # cosmetic badge accurate. 5s cadence matches _FILL_STATE_CACHE_TTL.
    try:
        if not getattr(app, '_fill_state_refresher_started', False):
            app._fill_state_refresher_started = True
            _t = threading.Thread(
                target=_fill_state_refresher,
                args=(app,),
                name='tm-fill-state-refresher',
                daemon=True)
            _t.start()
    except Exception:
        pass
    ...
    while not stop_event.is_set():
        try:
            ...
            if event_driven:
                ...
                # v4.14.6.94-event-idle-when-unactionable (Fix 3): the fast-poll
                # override pins the loop to 60s whenever a band is below target —
                # which keeps it "hot" even after Fix 1/2 have zeroed out the
                # discovery work and the market is closed (fill can't produce
                # genuinely-new picks with no fresh movement to analyze). When the
                # market is observed NOT live, skip the override so the existing
                # compute_backoff_sleep can relax the cadence to its designed idle
                # value (the same 60→600s the loop already targets when no
                # triggers fire). Liveness is only meaningful while Fix 1 is
                # actually running (it maintains _market_is_live), so we require
                # the market-gate sub-flag too. When the market IS live, OR any of
...
# ── v4.14.6.92-event-market-liveness-gate (Fix 1) ────────────────────────────
# Discovery triggers that re-scan price movement and therefore have NOTHING new
# to find when the market is closed (frozen prices). These are gated when the
# market is observed closed; the real-info triggers are NEVER gated.
_LIVENESS_GATED_KINDS = frozenset({
    'price_drift', 'volume_accumulation', 'fresh_universe_mover'})

# Data-driven market-liveness state. We do NOT trust the system clock or a
# holiday calendar; instead we observe whether prices are MOVING between sweeps
# (clock-independent, immune to surprise closures — frozen data looks closed
# regardless of WHY). yahoo_quote uses fast_info, which exposes no marketState,
# so the freshness/movement signal is the correct pure-gating path.
_MKT_PRICE_SNAPSHOT: dict = {}        # ticker -> last observed price (prev sweep)
_MKT_FROZEN_STREAK = [0]             # consecutive frozen sweeps
_MKT_IS_LIVE = [False]              # last computed liveness — FAIL-CLOSED default
_MKT_LOGGED_CLOSED = [False]        # one-shot "gated" log de-dup
_MKT_MOVE_EPS = 0.001              # |Δ| ≥ 0.1% counts as "moved"
_MKT_LIVE_MOVE_FRAC = 0.05         # ≥5% of tracked tickers moved → live
_MKT_LIVE_BIG_MOVE = 0.005         # …or any single ticker moved ≥ 0.5% → live
_MKT_FROZEN_SWEEPS_TO_CLOSE = 3   # sustained frozen sweeps (~3 min) → closed


def _update_market_liveness(app, all_fires) -> bool:
    """v4.14.6.92: derive 'is the market live right now' from price MOVEMENT
    between sweeps — purely from data already in hand (each price_drift fire
    carries its 'current' price in context). No system clock, no calendar, no
    new fetch, no AI. Frozen prices across consecutive sweeps ⇒ closed; movement
    ⇒ open. A 3-sweep frozen hysteresis absorbs the 60s quote-cache/60s-sweep
    aliasing (a single cache-stale sweep won't flip us closed during a live
    session). Fail-CLOSED until movement is actually observed. Never raises."""
    try:
        cur = {}
        for f in all_fires:
            try:
                ...
                continue
        # Snapshot the PRIOR sweep's prices BEFORE overwriting (a plain
        # reference would alias the live dict and zero out every delta).
        prev = dict(_MKT_PRICE_SNAPSHOT)
        common = [t for t in cur if t in prev and prev.get(t, 0) > 0]
        moved = 0
        max_move = 0.0
        for t in common:
            try:
                m = abs(cur[t] / prev[t] - 1.0)
                if m >= _MKT_MOVE_EPS:
                    moved += 1
                if m > max_move:
                    max_move = m
                ...
                continue
        # Update the snapshot for next sweep AFTER the delta is computed.
        if cur:
            _MKT_PRICE_SNAPSHOT.update(cur)
        if not common:
            # Nothing comparable yet (first sweep / disjoint sets) — can't
            # confirm movement, so keep the current (fail-closed) verdict.
            return _MKT_IS_LIVE[0]
        frac = moved / float(len(common))
        is_moving = (frac >= _MKT_LIVE_MOVE_FRAC) or (max_move >= _MKT_LIVE_BIG_MOVE)
        if is_moving:
            _MKT_FROZEN_STREAK[0] = 0
            _MKT_IS_LIVE[0] = True
            _MKT_LOGGED_CLOSED[0] = False
        else:
            _MKT_FROZEN_STREAK[0] += 1
            if _MKT_FROZEN_STREAK[0] >= _MKT_FROZEN_SWEEPS_TO_CLOSE:
                _MKT_IS_LIVE[0] = False
        return _MKT_IS_LIVE[0]
    except Exception:
        return _MKT_IS_LIVE[0]


def _market_is_live(app) -> bool:
    """Read the last-computed data-driven liveness verdict (fail-closed)."""
    try:
        return bool(_MKT_IS_LIVE[0])
    except Exception:
        return False
...
def _run_event_driven_sweep(app) -> int:
    ...
    # v4.14.6.92-event-market-liveness-gate (Fix 1): when the market is observed
    # CLOSED from the price DATA (prices frozen across sweeps), suppress the
    # price-movement DISCOVERY triggers so they stop re-scanning frozen
    # overnight/weekend prices and burning the shared provider caps. The
    # real-info triggers (user/target_stop/earnings/news/insider_buy/staleness)
    # ALWAYS pass through. Data-driven (no clock, no calendar), fail-closed,
    # flag-gated. Liveness is updated every sweep from the fires' own prices.
    try:
        _cfg = getattr(app, 'cfg', {}) or {}
        if (bool(_cfg.get('event_sweep_smart_gating', False))
                and bool(_cfg.get('event_gate_market_closed', False))):
            _update_market_liveness(app, all_fires)
            if not _market_is_live(app):
                _before = len(all_fires)
                all_fires = [f for f in all_fires
                             if f.get('kind') not in _LIVENESS_GATED_KINDS]
                _suppressed = _before - len(all_fires)
                if _suppressed and not _MKT_LOGGED_CLOSED[0]:
                    _MKT_LOGGED_CLOSED[0] = True
                    _log_muted(
                        app,
                        f"[event-driven] market observed CLOSED (prices frozen) "
                        f"— discovery triggers gated ({_suppressed} suppressed); "
                        f"real-info triggers (target_stop/news/earnings/insider) "
                        f"still active.")
                if not all_fires:
                    return 0
    except Exception:
        pass  # never let the gate break the sweep — fail open to prior behavior
    ...
    if _gate_on:
        ...
        if _bg:
            try:
                ...
                _can, _why = True, ''
            # v4.14.6.81-noai-board-works: in algo_only the background fires
            # route through the algo (AI dispatch suppressed, algo decides),
            # so "no AI providers" is NOT a reason to defer them — deferring
            # would starve the no-AI board. Strictly mode-gated; the AI path
            # defers exactly as before.
            try:
                if _resolve_tier1_mode(getattr(app, 'cfg', {}) or {}) == 'algo_only':
                    _can = True
            except Exception:
                pass
            if not _can:
...
def run_one_pass_for_triggers(app, path: str,
    ...
    # v4.14.6.82-noai-algo-emit: in algo_only the algo is the SOLE Tier-1
    # generator and needs NO AI picker — the unified dispatch below emits the
    # algo Tier-1 pick via _algo_gate_decide and suppresses the AI call (it
    # never dereferences `chosen` for a provider). So a failed AI pick (e.g.
    # none_configured at zero providers) must NOT bail the sweep — that's the
    # v81-missed gate. Swap in an algo_only sentinel `chosen` and fall through
    # to the dispatch so the algo emits. Strictly algo_only-gated: every AI
    # mode (and an AI mode with no USABLE AI — cooldown/exhausted/disabled)
    # still skips below, exactly as before.
    if (not chosen.get('success')) and _resolve_tier1_mode(
            getattr(app, 'cfg', {}) or {}) == 'algo_only':
        chosen = {'success': True, 'algo_only': True, 'reason': 'algo_only'}
        _log_muted(
            app,
            f"[{_src}] dispatch ({path}): no AI provider, algo_only "
            f"— proceeding with algo Tier-1 emit.")
    ...
    # "N analyzed via Groq" the user saw. The already-shipped honest wording
...
# ── MODE-SWITCH (tier1-mode-2026-06-14): the single control ──────────
#
# Three values, single source of truth:
#   'ai_only'      — AI scan ON, algo emit OFF, Tier-2 ON (legacy).
#   'ai_plus_algo' — AI scan OFF, algo emit ON, Tier-2 ON (DEFAULT).
#   'algo_only'    — AI scan OFF, algo emit ON, Tier-2 OFF.
#
# All Tier-1 / Tier-2 gating in this module routes through
# _resolve_tier1_mode(cfg) so the mapping lives in ONE place. Back-compat:
# when tier1_mode is absent (old config), the legacy `algo_tier1_emit`
# boolean is consulted — True → 'ai_plus_algo' (algo is sole Tier-1,
# Tier-2 stays on), False → 'ai_only' (pre-mode-switch behavior).
# v4.14.6.74-automatic-mode-toggle: 'automatic' is a SELECTABLE mode (and the
# new default) that resolves to an EFFECTIVE mode at resolve time — it is never
# returned to callers. With >=1 AI provider configured it resolves to
# 'ai_plus_algo' (algo tier-1 + AI tier-2); with zero AIs it resolves to
# 'algo_only' (algo runs standalone on cached data). This is what lets a
# zero-AI user get a usable board, and lets adding an AI auto-engage tier-2.
_VALID_TIER1_MODES = ('ai_only', 'ai_plus_algo', 'algo_only', 'automatic')
# The modes a caller can actually receive (automatic always resolves away).
_EFFECTIVE_TIER1_MODES = ('ai_only', 'ai_plus_algo', 'algo_only')


def _resolve_tier1_mode(cfg) -> str:
    """Return one of _EFFECTIVE_TIER1_MODES (never 'automatic'). Reads
    `cfg['tier1_mode']` first when present; otherwise falls back to legacy
    `algo_tier1_emit` for back-compat with pre-mode-switch configs. Always
    returns a valid EFFECTIVE mode string (unknown values fall through to the
    default, which is 'automatic' → resolved).

    'automatic' resolves by configured-provider COUNT (app-free, reads
    data/api_providers.json via tm_api_providers.load_enabled_providers):
    >=1 enabled provider → 'ai_plus_algo', else 'algo_only'. Fail-safe: if the
    provider read errors, treat as zero AIs → 'algo_only' (the standalone path
    that always works), never crash."""
    try:
        mode = str((cfg or {}).get('tier1_mode') or '').strip().lower()
    except Exception:
        mode = ''
    if mode == 'automatic':
        return _resolve_automatic_tier1_mode()
    if mode in _EFFECTIVE_TIER1_MODES:
        return mode
    # Legacy fallback: pre-mode-switch installs only had the boolean.
    try:
        legacy_emit = bool((cfg or {}).get('algo_tier1_emit', False))
    except Exception:
        legacy_emit = False
    if legacy_emit:
        return 'ai_plus_algo'
    # Explicit legacy "no emit" (the boolean is present and False) →
    # preserve the pre-mode-switch 'ai_only' behavior for that install.
    if cfg and ('algo_tier1_emit' in cfg):
        return 'ai_only'
    # Absent / unknown / unspecified → the NEW default is 'automatic'
    # (v4.14.6.74), resolved by provider count.
    return _resolve_automatic_tier1_mode()


def _resolve_automatic_tier1_mode() -> str:
    """v4.14.6.74: resolve 'automatic' by configured-provider COUNT.
    >=1 enabled AI provider → 'ai_plus_algo'; else → 'algo_only'. App-free
    (module-level provider read). Fail-safe to 'algo_only' (the standalone
    path that always produces a board) on any error — never raises."""
    try:
        import tm_api_providers as _tapi
        provs = _tapi.load_enabled_providers() or []
        return 'ai_plus_algo' if len(provs) >= 1 else 'algo_only'
    except Exception:
        return 'algo_only'
...
def _fill_mode_has_pending_work(app) -> bool:
    ...
    v4.14.6.42-ui-db-lock-fix: ALSO writes the answer into _fill_state_cache as
    a side effect (single source of truth). The Tk main thread reads that
    cache via has_pending_work_cached() lock-free instead of calling this
    function (which acquires app.db.lock via _fill_needed -> _path_actionable_
    displayed). All non-main callers (queue-runner _runner_loop tick @ line
    ~360, _fill_state_refresher daemon, internal fill code) keep populating
    the cache organically. Compute under app.db.lock as before — only the
    cache write is protected by the small private _fill_state_cache_lock."""
    _result = False
    try:
        if not bool((getattr(app, 'cfg', {}) or {}).get(
        ...
        # v4.14.6.42-ui-db-lock-fix: publish to the in-memory cache so the
        # Tk main thread can read this answer lock-free via
        # has_pending_work_cached(). Fail-silent (cache is cosmetic).
        try:
            _fill_state_cache_lock.acquire()
            try:
                _fill_state_cache['has_pending_work'] = bool(_result)
                _fill_state_cache['ts'] = time.time()
            finally:
                _fill_state_cache_lock.release()
        except Exception:
            pass
...
# ── v4.14.6.70-band-routing (Bug A) ──────────────────────────────────
# Tickers that miss the current path's band are ROUTED to the band their
# price fits (instead of only being discarded). band -> set(tickers); the
# target band consumes + clears these at the start of its next fill batch.
# Best-effort module state (mirrors _eligibility_skip_log_state); a rare
# race only drops/dupes a routed ticker, which dedup handles.
_routed_candidates: dict = {}


def price_to_band(app, price):
    """The band whose price range contains `price`, using the SAME _cfg
    min/max ranges the eligibility gate reads (config-driven, not hardcoded).
    Half-open [min, max) so a boundary price maps to exactly ONE band; the
    bottom band is unbounded below, the top band unbounded above. Returns a
    path key (e.g. 'band_50_up') or None. Never raises."""
    try:
        import tm_path_candidate_pools as _tpcp
        p = float(price)
        for _band in _tpcp._PATHS:
            cfg = _tpcp._cfg(app, _band) or {}
            lo = cfg.get('min_price_usd')
            hi = cfg.get('max_price_usd')
            lo_ok = (lo is None) or (p >= float(lo))
            hi_ok = (hi is None) or (p < float(hi))
            if lo_ok and hi_ok:
                return _band
        return None
    except Exception:
...
def take_routed_candidates(path) -> list:
    """Pop + return the tickers routed to `path` from prior wrong-band misses.
    They are in-band for `path` by construction (price_to_band placed them
    here), so they pass `path`'s eligibility and never re-route. Never raises."""
    try:
        r = _routed_candidates.pop(path, None)
...
def _eligibility_price_band_filter(app, path, candidates,
    ...
    for tk in cands:
        ...
        if (lo is not None and price < float(lo)) or \
            ...
            n_skip += 1
            # v4.14.6.70 Part 2: ROUTE, don't just drop. Re-queue this ticker
            # to the band its price actually fits (the score-100 EAT $160 fed
            # to moderate belongs in band_50_up). It enters its correct band's
            # feed next cycle, passes that band's eligibility (in-band by
            # construction), so it routes only ONCE — no loop.
            try:
                _dest = price_to_band(app, price)
                if _dest and _dest != path:
                    _routed_candidates.setdefault(_dest, set()).add(
                        str(tk).upper())
            except Exception:
                pass
...
def _layered_candidate_batch(app, path: str, limit: int = 20):
    ...
    if bool((getattr(app, 'cfg', {}) or {}).get(
        ...
        try:
            ...
            if pl:
                ...
                # v4.14.6.70 Part 1: the pool is empty/thin (unbuilt — a band
                # needs min_pool_size + min_price_history_days of cache). Do
                # NOT fail open to ALL prices (which floods the band with
                # wrong-price tickers that all get discarded). Fall back to a
                # DIRECT price-range filter on the path's own band, using the
                # SAME price source the eligibility gate reads. Cold-start
                # safe: a new user's banded paths get correctly-priced
                # candidates from day one instead of an all-price discard flood.
                _pcfg = _tpcp._cfg(app, path) or {}
                _band_lo = _pcfg.get('min_price_usd')
                _band_hi = _pcfg.get('max_price_usd')
        except Exception:
            pool = None
    ...
    def _inpool(seq):
        ...
        # Pool absent. If this is a price-banded path, restrict by its band
        # range (Part 1 fallback); seed-list / unbounded paths keep all.
        if _band_lo is None and _band_hi is None:
            return list(seq)
        out = []
        for t in seq:
            price = _latest_close_for(t)
            if price is None:
                out.append(t)  # fail-open on unknown price (same as the gate)
                continue
            if (_band_lo is not None and price < float(_band_lo)) or \
                    (_band_hi is not None and price > float(_band_hi)):
                continue       # out-of-band → don't feed this band
            out.append(t)
        return out
    ...
    batch: list = []
    # v4.14.6.70 Part 2: seed with tickers routed to THIS band from prior
    # wrong-band misses (recovered good picks like EAT). In-band by
    # construction; deduped + recency-excluded; capped to limit.
    for _rt in take_routed_candidates(path):
        if len(batch) >= limit:
            break
        if _rt not in batch and _rt not in skip:
            batch.append(_rt)
    ...
    t3 = []
    if len(batch) < limit:
        # v4.14.6.70 Part 1: wrap the universe-cursor tier through _inpool too
        # so the band-range fallback applies here when pool is None (restrict
        # passes the pool; _inpool adds the price-band guard on empty pools).
        t3 = _inpool(_build_candidate_shortlist(
...
def _run_fill_mode(app) -> None:
    ...
    try:
        ...
        # v4.14.6.46-coldstart-experience: cold-start honesty. Before fill mode
        # can pick anything, the per-ticker daily_bars cache needs ~100 tickers
        # minimum (the candidate tiers all require it). On a true cold start
        # that takes ~1 min for the v4.14.6.46 priority slice to land. Replace
        # the per-band "no candidates" spam with ONE calm waiting line per
        # cycle.
        try:
            import tm_cache as _tmcache_cs
            _scope_cs = _tmcache_cs.get_scope_tickers('daily_bars', None) or set()
            _unfilled_cs = _tmcache_cs.get_unfilled_tickers(
                'daily_bars', _scope_cs) or set()
            _filled_cs = len(_scope_cs) - len(_unfilled_cs)
            _COLDSTART_MIN_BARS = 100
            if _filled_cs < _COLDSTART_MIN_BARS:
                _last_cs_log = float(getattr(
                    app, '_fill_coldstart_last_log', 0) or 0)
                _now_cs = time.time()
                if (_now_cs - _last_cs_log) >= 30:
                    _log_routine(
                        app,
                        f"[fill-mode] waiting for first price data - "
                        f"{_filled_cs} of ~{_COLDSTART_MIN_BARS} tickers cached; "
                        f"board picks unlock as initial price history lands "
                        f"(usually ~1-2 min on first run)")
                    app._fill_coldstart_last_log = _now_cs
                return
        except Exception:
            pass
        ...
        if bool((getattr(app, 'cfg', {}) or {}).get(
            ...
            nowt = time.time()
            # v4.14.6.81-noai-board-works: in algo_only the algo IS the whole
            # engine and emits Tier-1 BUYs with NO provider — the unified-scan
            # suppresses the AI dispatch and lets the algo decide upstream, so
            # there is no "dispatch into the void" to protect against. The
            # circuit breaker (built for the AI-scan era) must therefore NOT
            # pause fill on no-/exhausted-providers in algo_only, or the board
            # never refills for a no-AI user. Strictly mode-gated: in any AI
            # mode (providers present), the pause is unchanged.
            try:
                _algo_only_fill = (_resolve_tier1_mode(
                    getattr(app, 'cfg', {}) or {}) == 'algo_only')
            except Exception:
                _algo_only_fill = False
            if _algo_only_fill:
                can_scan = True
            if not can_scan:
...
def _run_one_pass_body(app, rotation_path: str,
    ...
    _mark_runner_logged(app)
    # MODE-SWITCH (tier1-mode-2026-06-14): log the active mode once per
    # pass so the activity log is clear about which Tier-1 generator
    # is producing picks this cycle. Stays quiet on 'ai_only' (legacy
    # behavior, no need to call it out). Uses _log_amber so the line
    # survives `activity_log_verbosity=normal` — the prior _log_muted
    # version got collapsed by the routine-dedup machinery and the user
    # never saw it in the throwaway test.
    try:
        _ms_cfg_passlog = getattr(app, 'cfg', {}) or {}
        _ms_mode_passlog = _resolve_tier1_mode(_ms_cfg_passlog)
        if _ms_mode_passlog == 'ai_plus_algo':
            _log_amber(
                app,
                "[tier1-mode] ai_plus_algo: AI scan suppressed "
                "(algo is sole Tier-1); Tier-2 consensus still validates.")
        elif _ms_mode_passlog == 'algo_only':
            _log_amber(
                app,
                "[tier1-mode] algo_only: AI scan suppressed AND "
                "Tier-2 validation off — algo picks are final.")
    except Exception:
        pass
    ...
    # v4.14.6.82-noai-algo-emit: algo_only needs no AI picker (the unified
    # dispatch below emits the algo Tier-1 pick and suppresses the AI call). If
    # the pick failed only because there's no usable AI, swap in an algo_only
    # sentinel `chosen` so the pass proceeds to the algo emit instead of
    # skipping. Strictly algo_only-gated; AI modes (incl. an AI mode with no
    # usable AI) fall through to the existing pick-failure handling unchanged.
    if (not chosen.get('success')) and _resolve_tier1_mode(
            getattr(app, 'cfg', {}) or {}) == 'algo_only':
        chosen = {'success': True, 'algo_only': True, 'reason': 'algo_only'}
        _log_muted(
            app,
            f"Queue runner ({rotation_path}): no AI provider, algo_only "
            f"— proceeding with algo Tier-1 emit.")
...
def _algo_gate_decide(app, chosen: dict, ticker: str, path: str,
    ...
    # ALGOGATE (algo-selectivity-2026-06-14): default ON in this build.
    # Setting cfg['algo_selectivity_gate']=False is the rollback hook.
    selectivity_gate = bool(cfg.get('algo_selectivity_gate', True))
    # ENRICH (algo-enrichment-2026-06-14): default ON. Cfg rollback hook.
    enrichment_enabled = bool(cfg.get('algo_enrichment', True))
    survivor_threshold = float(cfg.get('algo_enrichment_survivor_threshold',
                                        55.0))
    # TREND (algo-trend-filter-2026-06-14): default ON. Cfg rollback hook.
    trend_filter_enabled = bool(cfg.get('algo_trend_filter', True))
    ...
    raw_tech = None
    raw_news = None
    raw_fund = None  # ALGOGATE
    raw_earnings = None  # ENRICH
    raw_insider = None   # ENRICH
    if cache is not None:
        ...
        try:
            ...
            raw_news = None
        # ALGOGATE: light fundamentals for the value-trap veto. Absent
        # → gate treats as not-deteriorating (absence ≠ bad).
        try:
            raw_fund = cache.fundamentals(ticker) or None
        except Exception:
            raw_fund = None
        # ENRICH: earnings calendar (next event date + estimates) and
        # insider flow aggregate. Both already cached, both USE IF PRESENT.
        try:
            raw_earnings = cache.earnings(ticker) or None
        except Exception:
            raw_earnings = None
        try:
            import tm_cache as _tc_if
            raw_insider = _tc_if.get_insider_flow(ticker) or None
        except Exception:
            raw_insider = None

    # ENRICH: stash current price into fundamentals dict so the
    # target-distance nudge can compute. Cheap; the dict is local.
    if raw_fund is not None and cache is not None:
        try:
            q = cache.quote(ticker) or {}
            cur_p = q.get('price')
            if cur_p is not None:
                # Shallow-copy so we don't mutate the shared cache dict.
                raw_fund = dict(raw_fund)
                raw_fund['_cur_price'] = cur_p
        except Exception:
            pass

    feats = _algo.normalize_features(
        raw_tech, raw_news, raw_fund,
        selectivity_gate=selectivity_gate,
        raw_earnings=raw_earnings,
        raw_insider_flow=raw_insider,
    ...
    # algo_skip_reason. The scorer writes _gate_rejected/_gate_reason
    # into the feature dict when the gate suppresses the dip bonus.
    gate_rejected = bool(feats.get('_gate_rejected'))
    gate_reason_text = str(feats.get('_gate_reason') or '')
    if gate_rejected:
        try:
            # TREND: distinguish trend-knife-block from selectivity-gate
            # rejection so the log line reads as the actual rule that
            # fired. The trend filter writes its gate_reason starting
            # with "trend:" — branch on that.
            if gate_reason_text.startswith('trend:'):
                _log_amber(
                    app,
                    f"[trend] {ticker}/{path} oversold but confirmed "
                    f"downtrend (px<SMA20<SMA50) — knife, oversold "
                    f"bonus suppressed; score={score}")
            else:
                _log_amber(
                    app,
                    f"[algo-gate] {ticker}/{path} oversold rejected: "
                    f"{gate_reason_text}; score={score} (dip bonus suppressed)")
        except Exception:
            pass
    ...
    # GATEFIX (algo-selectivity-2026-06-14, Fix 1): gate_rejected is
    # authoritative. When the gate flags a ticker as a knife (failed
    # bounce-confirm, news-negative, or fundamentals-deteriorating),
    # NO side-door promotion via bb_position-low / z-stretched / engulfing
    # / indicator-agreement is allowed. The gate exists so the algo can
    # be the new user's FRONT DOOR — a flagged knife must not land on
    # the board, period. Event-trigger bypass still respected (an
    # event-driven candidate is allowed through even if the gate would
    # have flagged it, matching pre-gate trigger-bypass semantics).
    if gate_rejected and not event_triggered:
        promote = False
        gate_reason = (
    ...
    #
    # When the algo decides to promote AND cfg['algo_tier1_emit'] is on,
    # emit a real predictions_log BUY record (status="open",
    # direction="BUY", source="algo_tier1", model="Algorithm") so
    # Tier-2/Layer-3 consensus engages exactly like an AI pick. Default
    # OFF — when off, behavior is byte-identical to today's shadow-only
    # algo. AI write path coexists, this flag ADDS the algo write
    # without replacing the AI write.
    #
    # Safety:
    #   - compute_algo_levels returns None on thin data → no emit, no
    #     fabricated stop.
    #   - dedupe: skip if an open BUY for (ticker, path) already exists
    #     in predictions_log — don't spawn a duplicate every cycle.
    #   - any failure inside this block is swallowed; the algo decision
    #     above is the source of truth, the emit is a side effect.
    # MODE-SWITCH (tier1-mode-2026-06-14): derive emit-on from
    # tier1_mode. Old configs without tier1_mode fall back to the
    # legacy algo_tier1_emit boolean for back-compat.
    emit_enabled = _resolve_tier1_mode(cfg) in (
        'ai_plus_algo', 'algo_only')
    emit_status = {
    ...
    if (emit_enabled and promote and not gate_rejected):
        emit_status['emit_attempted'] = True
        try:
            levels = _algo.compute_algo_levels(feats, cfg)
        except Exception as _e_lvl:
            levels = None
            emit_status['emit_skip_reason'] = (
                f'levels exception: {type(_e_lvl).__name__}')
        if levels is None and not emit_status['emit_skip_reason']:
            emit_status['emit_skip_reason'] = 'thin data (no ATR/price)'
        if levels is not None:
            emit_status['levels'] = levels
            _state = getattr(app, '_holdings_state', None) or {}
            _plog = _state.get('predictions_log')
            try:
                existing = None
                if _plog is not None:
                    existing = _plog.get_most_recent_for_ticker_and_path(
                        ticker, path)
                if (existing
                        and (existing.get('direction') or '').upper() == 'BUY'
                        and existing.get('status') in (None, '', 'open')):
                    emit_status['emit_skip_reason'] = (
                        f'open BUY already exists (id={existing.get("id")})')
                else:
                    score_int = int(round(float(score)))
                    conf_floor = str(
                        cfg.get('algo_tier1_emit_confidence', 'MODERATE')
                    ).upper().strip() or 'MODERATE'
                    if score_int >= 80:
                        confidence = 'HIGH'
                    elif score_int >= 65:
                        confidence = (
                            'MODERATE' if conf_floor in (
                                'MODERATE', 'LOW') else 'HIGH')
                    else:
                        confidence = conf_floor
                    raw_text_lines = [
                        f"DIRECTION: BUY",
                        f"BUY_ZONE: ${levels['buy_zone_low']:.2f} - "
                        f"${levels['buy_zone_high']:.2f}",
                        f"TARGET: ${levels['target']:.2f}",
                        f"STOP_LOSS: ${levels['stop']:.2f}",
                        f"TIMEFRAME: {levels['timeframe_days']} days",
                        f"CONFIDENCE: {confidence}",
                        f"",
                        f"Algorithmic Tier-1 pick (rule-based scorer). "
                        f"Score {score:.1f} / threshold {threshold:.1f}. "
                        f"RR {levels['rr_ratio']:.1f}:1. "
                        + (f"{levels['notes']}. " if levels['notes'] else "")
                        + "Rules that fired:",
                    ]
                    for _r in (reasons or [])[:12]:
                        raw_text_lines.append(f"  - {_r}")
                    raw_text = '\n'.join(raw_text_lines)
                    rec = {
                        'ticker':                       ticker,
                        'direction':                    'BUY',
                        ...
                        'stop':                         levels['stop'],
                        'timeframe_days':               levels['timeframe_days'],
                        'confidence':                   confidence,
                        'current_price_at_prediction':  levels['inputs']['price'],
                        'raw_text':                     raw_text,
                        'path':                         path,
                        'source':                       'algo_tier1',
                        'model':                        'Algorithm',
                        'provider_id':                  None,
                        'canonical_model':              'algo',
                        'actual_provider':              'algo',
                        'actual_model_string':          'algo_tier1_v1',
                        'lineup_version':               'algo_tier1_v1',
                        'data_version':                 'v4.14.1',
                        # v4.14.6.69-display-validation-rank: persist the raw
                        # algo-tier1 score numerically so refresh_recommend_cache
                        # can rank this BUY on its real score instead of sinking
                        # it to 0.0 when the momentum filter didn't score it.
                        'algo_score':                   float(score),
                    }
                    if _plog is not None:
                        pred_id = _plog.append(rec)
                        emit_status['emit_written'] = True
                        emit_status['emit_pred_id'] = pred_id
                        # drainfix-2026-06-15 (v4.14.6.40): mirror the AI
                        # dispatch path's _record_analysis_outcome call so
                        # this algo BUY also lands in
                        # queue_runner_analysis_log. That makes the
                        # EXISTING recency-skip in _recently_judged_set
                        # (L2211-2304) protect algo BUYs the same way it
                        # already protects AI BUYs — the ticker won't be
                        # re-scanned (and re-litigated by the candidate
                        # prompt) within the path's staleness window
                        # (5 days, STALENESS_WINDOWS_SECONDS_STABLE).
                        # Without this, algo BUYs were freely re-scannable
                        # after the 3-day maturity guard, and any
                        # cross-model AVOID marked them contradicted → the
                        # board drained overnight. Best-effort: any error
                        # here is swallowed; the emit itself is the
                        # primary outcome.
                        try:
                        ...
                        try:
                            _log_amber(
                                app,
                                f"[algo-tier1] emitted BUY {ticker}/{path} "
                                f"entry [{levels['buy_zone_low']:.2f}-"
                                f"{levels['buy_zone_high']:.2f}] "
                                f"target {levels['target']:.2f} "
                        ...
                        emit_status['emit_skip_reason'] = (
                            'predictions_log not available')
            except Exception as _e_emit:
                emit_status['emit_skip_reason'] = (
                    f'emit exception: {type(_e_emit).__name__}: {_e_emit}')
    elif emit_enabled and not promote:
        emit_status['emit_skip_reason'] = 'promote=False'
    elif emit_enabled and gate_rejected:
        emit_status['emit_skip_reason'] = 'gate_rejected'
    ...
    return {
        ...
        'threshold': threshold,
        # ALGOGATE outputs (used by _algo_log_shadow → algo_skip_reason).
        'gate_rejected': gate_rejected,
        'gate_reject_reason': gate_reason_text,
        'fund_available': bool(feats.get('_fund_available')),
        'news_available': bool(feats.get('_news_available')),
        # ENRICH outputs — track which factors had data and which fired
        # so the shadow log can show how picks change as the cache fills.
        'enrich_enabled': bool(feats.get('_enrich_enabled')),
        'enrich_ran_expensive': bool(feats.get('_enrich_ran_expensive')),
        'enrich_factors_present': list(
            feats.get('_enrich_factors_present') or ()),
        'enrich_factors_available': list(
            feats.get('_enrich_factors_available') or ()),
        'earnings_available': bool(raw_earnings),
        'insider_available': bool(raw_insider),
        # TREND outputs — the filter's state + data presence so we can
        # see knife suppression vs reclaim-allow vs unknown-skip in the
        # shadow log.
        'trend_state': str(feats.get('_trend_state') or 'unknown'),
        'tech_has_sma20': bool(feats.get('_tech_has_sma20')),
        'tech_has_sma50': bool(feats.get('_tech_has_sma50')),
        'trend_filter_enabled': bool(
            feats.get('_trend_filter_enabled')),
        # EMIT-BRIDGE outputs — visible per-record in the shadow log so
        # we can see emit attempts, skips (with reason), and successes
        # alongside the algo decision.
        'emit_enabled':     bool(emit_status['emit_enabled']),
        'emit_attempted':   bool(emit_status['emit_attempted']),
        'emit_written':     bool(emit_status['emit_written']),
        'emit_skip_reason': str(emit_status['emit_skip_reason'] or ''),
        'emit_levels':      emit_status.get('levels'),
        'emit_pred_id':     emit_status.get('emit_pred_id', ''),
    }
...
def _algo_log_shadow(app, ticker: str, path: str, decision: dict,
    ...
    try:
        ...
        # ALGOGATE (algo-selectivity-2026-06-14): derive algo_skip_reason
        # inline so each shadow record carries its cause without needing
        # an activity-log cross-reference. Diagnostic only — does not
        # change pick logic.
        # GATEFIX (Fix 2, 2026-06-14): gate_rejected wins the label.
        # If the gate flagged the ticker as a knife, the record reads
        # 'gate_rejected_knife' regardless of whether score cleared
        # threshold or whether algo_would_promote ended up True. The
        # label must never lie ("promoted" on something the gate killed).
        # After Fix 1 these are consistent anyway (gate-rejected always
        # forces promote=False outside event-trigger), but the label
        # ordering stays correct even if some future change re-introduces
        # a side-door promote.
        _promoted = bool(decision.get('algo_would_promote'))
        _gate_rej = bool(decision.get('gate_rejected'))
        _score    = decision.get('score')
        _thr      = decision.get('threshold')
        if _gate_rej:
            _skip_reason = 'gate_rejected_knife'
        elif not ai_ran:
            _skip_reason = 'no_eligible_providers'
        elif _promoted:
            _skip_reason = 'promoted'
        elif (_score is not None and _thr is not None
              and float(_score) < float(_thr)):
            _skip_reason = 'score_below_threshold'
        else:
            _skip_reason = 'other'
        # v4.14.6.66-algo-momentum-align: best-effort join to the Layer-2
        # consensus verdict for THIS (ticker, path), so the shadow log can
        # measure algo↔AI BUY agreement WITHOUT switching to ai_only mode (in
        # the default ai_plus_algo the tier-1 AI scan is suppressed, so
        # ai_direction above is usually null). A freshly-emitted pick has no
        # verdict yet — the 180s Layer-2 daemon validates AFTER emit — but the
        # scan runs continuously, so on later cycles the verdict populates and
        # a few days of shadow data yields a real agreement number vs the ZERO
        # baseline. Read-only, fail-open null; never blocks dispatch.
        _l2_verdict = None
        _l2_agreement = None
        _l2_nmodels = None
        try:
            ...
            if _l2_conn is not None:
                with _db_lock(app):
                    _l2_row = _l2_conn.execute(
                        "SELECT verdict, agreement_pct, n_models "
                        "FROM recommend_cache_validation "
                        "WHERE ticker = ? AND path = ?",
                        (str(ticker).upper(), str(path))).fetchone()
                if _l2_row is not None:
                    _l2_verdict   = _l2_row[0]
                    _l2_agreement = _l2_row[1]
                    _l2_nmodels   = _l2_row[2]
        except Exception:
            _l2_verdict = _l2_agreement = _l2_nmodels = None
        record = {
            ...
            # ALGOGATE fields
            'algo_skip_reason':   _skip_reason,
            'gate_rejected':      _gate_rej,
            'gate_reject_reason': decision.get('gate_reject_reason', ''),
            'fund_available':     bool(decision.get('fund_available')),
            'news_available':     bool(decision.get('news_available')),
            # ENRICH fields — show how data availability + enrichment
            # factors evolve as the new-user cache fills.
            'enrich_enabled':           bool(decision.get('enrich_enabled')),
            'enrich_ran_expensive':     bool(decision.get('enrich_ran_expensive')),
            'enrich_factors_present':   list(decision.get('enrich_factors_present') or []),
            'enrich_factors_available': list(decision.get('enrich_factors_available') or []),
            'earnings_available':       bool(decision.get('earnings_available')),
            'insider_available':        bool(decision.get('insider_available')),
            # TREND fields — read on each record to see knife suppression
            # working vs reclaim-allow vs unknown-skip evolution.
            'trend_state':              str(decision.get('trend_state') or 'unknown'),
            'tech_has_sma20':           bool(decision.get('tech_has_sma20')),
            'tech_has_sma50':           bool(decision.get('tech_has_sma50')),
            'trend_filter_enabled':     bool(decision.get('trend_filter_enabled')),
            # EMIT-BRIDGE fields — show every emit attempt + outcome.
            'emit_enabled':             bool(decision.get('emit_enabled')),
            'emit_attempted':           bool(decision.get('emit_attempted')),
            'emit_written':             bool(decision.get('emit_written')),
            'emit_skip_reason':         str(decision.get('emit_skip_reason') or ''),
            'emit_levels':              decision.get('emit_levels'),
            'emit_pred_id':             str(decision.get('emit_pred_id') or ''),
            # v4.14.6.66 Layer-2 consensus join — the algo↔AI agreement metric.
            # null until the L2 daemon validates this (ticker, path); populates
            # on later scan cycles. Verdicts: VALIDATED_BUY / MIXED /
            # CONTRADICTED / SINGLE_VOICE / INCONCLUSIVE.
            'layer2_verdict':           _l2_verdict,
            'layer2_agreement_pct':     _l2_agreement,
            'layer2_n_models':          _l2_nmodels,
        }
...
def _run_cloud_multi(app, prompt, ticker, eligible_paths, chosen,
    ...
    # ── v4.14.6.37: algorithmic tier-1 shadow gate (UNIFIED-SCAN PATH) ──
    #
    # The v4.14.6.31 shadow integration was wired into _analyze_candidate
    # only — but ~100% of real tier-1 traffic flows through this
    # unified-scan path (`_run_cloud_multi`). The v4.14.6.31-36 install
    # logged ZERO shadow records over hours of dispatch because the
    # gate never fired here. This patch adds the gate so every
    # unified-scan dispatch contributes to the A/B telemetry.
    #
    # Behavior in shadow mode (current default):
    #   * COMPUTE the algo decision for the ticker (technical features
    #     are path-agnostic; we score once per call).
    #   * Do NOT short-circuit; the AI dispatch runs exactly as today.
    #   * After dispatch, emit ONE shadow record per (ticker, path) so
    #     the eventual A/B comparison is per-path — paths have
    #     different band semantics + may parse different AI outcomes.
    #
    # Behavior in live mode (`use_algorithmic_tier1=True`): same compute,
    # same per-path log. We do NOT add a multi-path live-skip here —
    # the live-skip is a separate decision and we don't have
    # per-path-aware short-circuit semantics for the unified call yet.
    # The per-(ticker, path) shadow data is the prerequisite for that
    # future work.
    #
    # Fail-safe: any exception in algo compute or shadow write is
    # swallowed; the real AI dispatch must NEVER be affected by the
    # shadow path.
    _us_cfg = getattr(app, 'cfg', None) or {}
    _us_algo_live = bool(_us_cfg.get('use_algorithmic_tier1', False))
    _us_algo_shadow = bool(_us_cfg.get('algo_tier1_shadow', True))
    _us_algo_decision = None  # populated when the gate runs
    if _us_algo_live or _us_algo_shadow:
        try:
            _us_algo_decision = _algo_gate_decide(
                app, {}, ticker, primary, _us_cfg)
        except Exception as _e_algo:
            _us_algo_decision = {
                'error': f'{type(_e_algo).__name__}: {_e_algo}',
                'algo_would_promote': True,  # safe-fail
                'score': None, 'reasons': [],
            }

    def _emit_shadow_records(per_path_preds: Optional[dict]) -> None:
        """v4.14.6.37: emit one shadow record per (ticker, path).

        per_path_preds is either:
          * None / {} → dispatch failed before any AI outcome reached
            us. Emit ONE record for the primary path with ai_ran=False.
          * dict mapping path → prediction dict (or None) → emit ONE
            record per path. ai_ran is True when the prediction dict
            is present; the per-path direction drives ai_promoted.
        """
        if _us_algo_decision is None:
            return  # gate didn't run (shadow + live both off)
        try:
            if per_path_preds:
                for _p, _pred in per_path_preds.items():
                    try:
                        _algo_log_shadow(
                            app, ticker, _p, _us_algo_decision,
                            ai_ran=(_pred is not None),
                            ai_prediction=(_pred
                ...
                # Dispatch failed entirely — record the algo decision
                # against the primary path with ai_ran=False so the
                # A/B summary still sees the algo's would-promote
                # state (and counts the candidate as something the
                # AI tried but never produced a usable outcome on).
                try:
                    _algo_log_shadow(
                        app, ticker, primary, _us_algo_decision,
                        ai_ran=False, ai_prediction=None)
                except Exception:
                    pass
        except Exception:
            pass  # logging must NEVER raise into the dispatch path
    ...
    try:
        ...
        before = 0
    _rmeta: dict = {}
    # MODE-SWITCH (tier1-mode-2026-06-14): the AI scan dispatch only
    # runs in 'ai_only' mode. In 'ai_plus_algo' / 'algo_only' the algo
    # path above is the sole Tier-1 generator and the per-candidate AI
    # call is suppressed. The shadow record fan-out below (and the
    # empty-result control flow) already handles the "no AI result"
    # case identically to a provider-unavailable bail, so this gate
    # composes cleanly with the rest of _run_cloud_multi without
    # restructuring. The one-line-per-pass log gate lives at the queue
    # runner level; here we set a local flag and skip the dispatch.
    _ms_mode = _resolve_tier1_mode(_us_cfg)
    _ms_ai_scan_on = (_ms_mode == 'ai_only')
    try:
        def _router_log(msg, color='muted'):
            _safe_log(app, msg, color)
        if _ms_ai_scan_on:
            ...
            # Algo-as-Tier-1 mode: skip the AI dispatch. The algo
            # decision + (when emit is on) the algo BUY have already
            # been written upstream in _algo_gate_decide. Emit the
            # shadow record with ai_ran=False and stamp the cause so
            # the caller can log "ai_scan_suppressed" instead of
            # falling through to the uncategorized "unparseable" line.
            _stamp('ai_scan_suppressed', mode=_ms_mode)
            _emit_shadow_records(None)
            return {}
    except Exception as e:
        ...
        try:
            ...
            pass
        _emit_shadow_records(None)  # v4.14.6.37 shadow: dispatch failed
        return {}
    # v4.14.5.62-concurrent-scan: prefer the record the router handed back
    ...
    if primary_rec is None:
        try:
            ...
            _emit_shadow_records(None)  # v4.14.6.37 shadow
            return {}
        if not new_rows:
            if _rmeta.get('provider_unavailable'):
    ...
    # v4.14.6.37 shadow: emit one record per (ticker, path) in result.
    # If a path appeared in `result`, the AI produced an outcome for
    # it. If `result` is empty (e.g. primary's direction was missing
    # AND no other paths parsed), the next line writes the algo
    # decision against the primary path with ai_ran=False so the
    # candidate still appears in the A/B telemetry.
    _emit_shadow_records(result if result else None)
    return result
        ...
        if not res:
            ...
            if _cause == 'parsed_no_direction':
                ...
                _log_amber(
                    ...
                    f"next cycle.")
            elif _cause == 'ai_scan_suppressed':
                # MODE-SWITCH (tier1-mode-2026-06-14): the AI dispatch
                # was intentionally skipped by the active mode. Log
                # muted (this happens for every candidate in non-
                # ai_only modes, no need to amber/spam) and skip the
                # legacy uncategorized "unparseable" line — the
                # behavior is by design, not an error.
                _mode_lbl = _status.get('mode') or '?'
                _log_muted(
                    app,
                    f"[unified-scan] {ticker}: AI scan suppressed "
                    f"(tier1_mode={_mode_lbl}); algo decided "
                    f"upstream.")
            else:
                _log_amber(
                    app,
...
def _emit_summary_log(app, chosen: dict, candidate_count: int,
    ...
    if outcome == 'no_candidates':
        ...
        if candidate_count == 0:
            ...
            # MODE-SWITCH (tier1-mode-2026-06-14): in 'ai_plus_algo' and
            # 'algo_only' the per-candidate AI dispatch is suppressed,
            # so the legacy "{N} analyzed via {provider}" wording lied —
            # the rotation pick was made but the AI was never actually
            # called. Swap the phrase so the suppression is legible.
            _ms_cfg_sum = getattr(app, 'cfg', {}) or {}
            _ms_mode_sum = _resolve_tier1_mode(_ms_cfg_sum)
            _ms_ai_scan_off = (_ms_mode_sum != 'ai_only')
            segs = [_cand]
            if gt:
                segs.append(f"{gt} gated")
            if _ms_ai_scan_off:
                # Honest accounting: the dispatch path saw N candidates
                # but the AI call was suppressed by the active mode. The
                # algo emit is still counted under _picks_queued.
                segs.append(
                    f"{analyzed} routed (AI scan suppressed by "
                    f"tier1_mode={_ms_mode_sum})")
            else:
                segs.append(
                    f"{analyzed} analyzed"
