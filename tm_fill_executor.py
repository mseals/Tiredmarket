...
# v4.14.6.46-coldstart-experience: priority slice prepended to the
# chunked daily_bars fetch order on a true cold start. Covers all four
# price bands so fill mode has analyzable tickers within ~1 minute
# instead of ~30+ for the full ~7200 universe.
_COLDSTART_PRIORITY_FALLBACK = [
    # Mega/large-cap ($50+): always available, deep history, broad coverage.
    'AAPL', 'MSFT', 'NVDA', 'AMZN', 'GOOGL', 'META', 'TSLA', 'BRK-B', 'LLY', 'UNH',
    'JPM', 'V', 'XOM', 'PG', 'MA', 'HD', 'COST', 'BAC', 'WMT', 'NFLX',
    # Mid-cap ($10-50): liquid, ETFs/banks/industrials.
    'AAL', 'SOFI', 'F', 'T', 'PFE', 'GE', 'CCL', 'UBER', 'SNAP',
    'NIO', 'LCID', 'HOOD', 'RIVN', 'PLTR', 'AMD', 'INTC', 'DAL', 'VZ', 'GOLD',
    # Small-cap / $5-10: penny-ish liquid names with daily volume.
    'PLUG', 'BB', 'NOK', 'MARA', 'RIOT', 'SIRI', 'AMC', 'BBIG', 'SOXS', 'UVXY',
    # Sub-$5 / lottery: high-volume penny names.
    'IDEX', 'SNDL', 'CTRM', 'OCGN', 'NAKD', 'GNUS', 'TLRY', 'HEXO', 'BTBT', 'EBON',
]


def _coldstart_priority_slice(app=None) -> list:
    """v4.14.6.47-coldstart-autostart-fix: return a de-duplicated priority
    slice of ~400-600 tickers spanning all four price bands. The dead
    iShares JSON paths (universe_ivv/iwb/iwm/nasdaq100.json) are gone —
    iShares stopped serving those files months ago. New strategy:

      1. Always seed the 49-ticker hardcoded fallback first (true
         cold-cold safety net — spans all four price bands).
      2. Then expand from the REAL current universe via
         tm_news_fetcher._universe(app) or
         tm_cache.get_scope_tickers('daily_bars', None), bucketed by
         price band using quote_cache.json when it's warm.
      3. If quote_cache.json is absent (true cold), still append a head
         slice of the live universe — bars start landing for those names
         on the very first chunks regardless.

    Returns UPPER-case tickers in priority order, capped at 600."""
    seen = set()
    out = []
    ...
    def _add(t):
        ...
        if u and u not in seen:
            seen.add(u)
            out.append(u)
    # Hardcoded fallback always first - guarantees all bands seeded
    # even on the coldest of cold starts.
    for t in _COLDSTART_PRIORITY_FALLBACK:
        _add(t)
    # Expand from the real SEC-derived universe + cached quotes.
    try:
        # Source 1: app-attached universe (post-discover refresh).
        universe = None
        if app is not None:
            try:
                import tm_news_fetcher as _tnf
                universe = list(_tnf._universe(app) or [])
            except Exception:
                universe = None
        # Source 2: post-seed scope (works once tm_cache has seen any
        # tickers under the daily_bars lane).
        if not universe:
            try:
                import tm_cache as _tc
                universe = list(
                    _tc.get_scope_tickers('daily_bars', None) or [])
            except Exception:
                universe = []
        # Price bands from quote_cache.json (if quotes are warm).
        prices = {}
        try:
            import json as _json
            here = Path(__file__).resolve().parent
            qcp = __import__('tm_paths').get_data_dir() / 'quote_cache.json'
            if qcp.exists():
                with open(qcp, 'r', encoding='utf-8') as f:
                    raw = _json.load(f)
                for tk, blob in (raw or {}).items():
                    try:
                        if isinstance(blob, dict):
                            ...
                            p = blob
                        if p is not None:
                            prices[str(tk).upper()] = float(p)
                    except Exception:
                        pass
        except Exception:
            pass
        if universe and prices:
            buckets = {'b50': [], 'b1050': [], 'b510': [], 'lott': []}
            for tk in universe:
                u = str(tk).upper().strip()
                if not u:
                    continue
                p = prices.get(u)
                if p is None:
                    continue
                if p >= 50.0:
                    buckets['b50'].append(u)
                elif p >= 10.0:
                    buckets['b1050'].append(u)
                elif p >= 5.0:
                    buckets['b510'].append(u)
                elif p > 0:
                    buckets['lott'].append(u)
            per_band = 150
            keys = ('b50', 'b1050', 'b510', 'lott')
            for k in keys:
                buckets[k] = buckets[k][:per_band]
            max_len = max(len(buckets[k]) for k in keys) if any(
                buckets[k] for k in keys) else 0
            for i in range(max_len):
                for k in keys:
                    if i < len(buckets[k]):
                        _add(buckets[k][i])
        elif universe:
            # No price info → take a head slice of the universe; bars
            # will start landing for these on the first chunks anyway.
            for tk in universe[:400]:
                _add(tk)
    except Exception:
        pass
    # Cap to a reasonable size - large enough to span bands, small enough
    # to complete in ~5-6 chunks (~1 minute).
    return out[:600]
...
def _run_chunked_phase(lane: str, unfilled_tickers, chunked_fetcher,
    ...
    # v4.14.6.63-fill-quarantine (S1): pre-filter tickers under an
    # unexpired quarantine window so they consume NO bulk slot and NO
    # per-ticker fallback slot. Diagnostic showed ~98% of the
    # historical "need per-ticker fallback" cohort is a persistent
    # never-fillable set (warrants/units/preferred + delisted residue).
    # Tickers whose retry window has just opened ARE included in the
    # bulk submission (they get ONE attempt); record_bulk_outcome below
    # decides whether they're cleared or re-quarantined.
    submitted_pre_quarantine = list(unfilled_tickers)
    skipped_q: list[str] = []
    retry_window: list[str] = []
    try:
        import tm_quarantine as _q
        if lane == 'daily_bars':
            (submitted_pre_quarantine, skipped_q, retry_window
                ) = _q.filter_eligible(lane, submitted_pre_quarantine)
            if skipped_q:
                # Tease apart upcoming retry-window dates for the log.
                _snap = _q.status_snapshot(lane).get(
                    'quarantined_until', {})
                _next_dates = sorted({_snap[t] for t in skipped_q
                                       if t in _snap})
                _next_repr = _next_dates[0] if _next_dates else '?'
                _fill_log(
                    log_callback,
                    f"[{lane_prefix}] {lane} phase 1: quarantine "
                    f"skipped {len(skipped_q)} ticker(s) "
                    f"(N=3 consecutive bulk-fails, 30-day window); "
                    f"next retry-window {_next_repr}",
                    'muted')
            if retry_window:
                _fill_log(
                    log_callback,
                    f"[{lane_prefix}] {lane} phase 1: granting one "
                    f"retry-window attempt to {len(retry_window)} "
                    f"previously-quarantined ticker(s)",
                    'muted')
    except Exception:
        # Fail-safe: any quarantine error → behave like v62 (no filter).
        submitted_pre_quarantine = list(unfilled_tickers)
        skipped_q = []
        retry_window = []

    # v4.14.6.46-coldstart-experience / v4.14.6.53: prepend the priority
    # slice so the first ~5-6 chunks cover tickers across all four price
    # bands. Designed for true cold-start but fires in warm steady-state
    # too (priority-first ordering is harmless either way). Full universe
    # still fetched after; only the order changes.
    tickers = list(submitted_pre_quarantine)
    if lane == 'daily_bars':
        try:
            prio = _coldstart_priority_slice(app=app)
            # Keep ONLY priority tickers that are actually in the unfilled set.
            prio_set = set(prio)
            unfilled_set = set(tickers)
            prio_in = [t for t in prio if t in unfilled_set]
            rest = [t for t in tickers if t not in prio_set]
            tickers = prio_in + rest
            _fill_log(
                log_callback,
                f"[{lane_prefix}] {lane} phase 1: priority slice "
                f"prepended ({len(prio_in)} tickers across all "
                f"price bands)", 'muted')
        except Exception:
            pass
    total = len(tickers)
    ...
    # v4.14.6.63-fill-quarantine (S1): increment fail counter for every
    # bulk-submitted ticker that did not land in `filled`; reset for
    # successes; auto-quarantine at N=3 consecutive bulk-fails. Daily-
    # bars only — other lanes don't have the persistent ghost-list
    # pattern. Single log line summarises new quarantines / clears.
    if lane == 'daily_bars':
        try:
            import tm_quarantine as _q
            summary = _q.record_bulk_outcome(
                lane, tickers, filled)
            if summary.get('newly_quarantined'):
                _fill_log(
                    log_callback,
                    f"[{lane_prefix}] {lane} phase 1: quarantined "
                    f"{len(summary['newly_quarantined'])} ticker(s) "
                    f"after 3 consecutive bulk-fails — they will be "
                    f"skipped for 30 days",
                    'muted')
            if summary.get('retry_failed_requarantined'):
                _fill_log(
                    log_callback,
                    f"[{lane_prefix}] {lane} phase 1: "
                    f"{len(summary['retry_failed_requarantined'])} "
                    f"retry-window attempt(s) failed — re-quarantined "
                    f"for 30 days",
                    'muted')
            if summary.get('cleared'):
                _fill_log(
                    log_callback,
                    f"[{lane_prefix}] {lane} phase 1: cleared "
                    f"quarantine for "
                    f"{len(summary['cleared'])} ticker(s) "
                    f"(successful retry)",
                    'accent')
        except Exception:
            pass
    ...
    def _worker():
        try:
            ...
            for lane in tm_cache.BULK_FILLABLE_LANES:
                ...
                # v4.14.6.63-fill-quarantine (S1): also drop quarantined
                # tickers from the per-ticker fallback loop. The chunked
                # phase already pre-filtered them, but the post-chunked
                # `get_unfilled_tickers` recompute re-includes them
                # (the freshness gate is unaware of quarantine — by
                # design, the v62 freshness semantics are untouched).
                # Filtering here closes the loop so quarantined tickers
                # consume NEITHER bulk slots NOR slow per-ticker slots.
                if lane == 'daily_bars':
                    try:
                        import tm_quarantine as _q
                        _eligible, _skipped_q, _retry_q = _q.filter_eligible(
                            lane, list(unfilled))
                        if _skipped_q:
                            unfilled = set(_eligible) | set(_retry_q)
                    except Exception:
                        pass
    ...
    if pass_count == 1:
        ...
        if _cf is not None:
            ...
            chunk_filled = _run_chunked_phase(
                ...
                log_callback, stop_event, lane_prefix='data', app=app)
            if chunk_filled:
