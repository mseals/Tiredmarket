...
class PortfolioPanel:
    ...
    def _build_summary_card(self, parent, portfolio):
        ...
        tk.Label(card, text=breakdown, bg=c['card2'], fg=c['muted'],
    ...
    def _build_tradable_card(self, parent, holding):
        ...
        if current_price is not None and buy_price:
            ...
            tk.Label(line1, text=pos_text, bg=c['card'], fg=c['muted'],
                     font=self._card_fonts['body']).pack(side='left', padx=(s['sm'], 0))
            tk.Label(line1,
                     text=f"  {sign}{_format_money(pnl)} ({sign}{pnl_pct:.1f}%)",
                     bg=c['card'], fg=pnl_color,
                     font=self._card_fonts['body_bold']).pack(side='left')
        else:
            tk.Label(line1,
    ...
    def _latest_price(self, ticker):
        """Return the current price from the cache, or None if unknown.
        Never blocks for a fresh fetch — uses what's already cached."""
        cache = self._cache()
        if cache is None:
            return None
        try:
