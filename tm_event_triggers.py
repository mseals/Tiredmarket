...
# ── v4.14.6.93-price-drift-change-dedup (Fix 2) ──────────────────────────────
# A real materiality floor + change-based dedup for price_drift. Today the
# per-path threshold is effectively 0% and the only re-fire gate is a 30-min
# TIME window, so an unchanged ticker re-fires (and re-dispatches an AI scan)
# every 30 min purely because time elapsed. Pure arithmetic — 0 AI, 0 network,
# 0 schema change (reuses the `current` already stored in trigger_fire_log.
# signal_context). Flag-gated under the master event_sweep_smart_gating.
_PRICE_DRIFT_MIN_PCT = 0.02        # 2% materiality floor — trivial/0% drift won't fire
_PRICE_DRIFT_RECHECK_PCT = 0.02    # must move ≥2% since LAST ANALYZED to re-fire


def _moved_since_last_fire(conn, ticker: str, path: str, kind: str,
                            current: float, recheck_pct: float) -> bool:
    """v4.14.6.93: True iff the price moved ≥ recheck_pct since this ticker+kind
    was LAST analyzed (the `current` recorded on its most-recent
    trigger_fire_log row) — i.e. there is genuinely NEW movement worth a
    re-scan, not just elapsed time. Reuses the signal_context already written;
    no new column. Fallback to True (eligible — preserve prior behavior) when
    there's no prior row, no stored `current`, or any parse/DB error. Never
    raises."""
    if conn is None:
        return True
    try:
        cur = conn.execute(
            "SELECT signal_context FROM trigger_fire_log "
            "WHERE ticker = ? AND path = ? AND trigger_kind = ? "
            "ORDER BY fired_at DESC LIMIT 1",
            (ticker, path, kind))
        row = cur.fetchone()
        if not row or not row[0]:
            return True                     # no prior fire / no context → eligible
        last_current = None
        try:
            ctx = json.loads(row[0])
            last_current = ctx.get('current')
            last_current = (float(last_current)
                            if last_current is not None else None)
        except Exception:
            last_current = None
        if not last_current or last_current <= 0 or current <= 0:
            return True                     # missing/invalid → fall back to eligible
        return abs(current / last_current - 1.0) >= recheck_pct
    except Exception:
        return True
...
def check_price_triggers(app, path: str,
    ...
    with _db_lock(app):
        ...
        # v4.14.6.93-price-drift-change-dedup (Fix 2): when on, apply a 2%
        # materiality FLOOR (so trivial/0% drift never fires) and require real
        # movement since LAST analyzed (change-based dedup). Flag-gated under the
        # master; OFF => exact prior behavior (0%-floor, time-only dedup).
        _cfg = getattr(app, 'cfg', {}) or {}
        _pd_change_dedup = (
            bool(_cfg.get('event_sweep_smart_gating', False))
            and bool(_cfg.get('event_dedup_price_change', True)))
        _eff_threshold = (max(threshold, _PRICE_DRIFT_MIN_PCT)
                          if _pd_change_dedup else threshold)
        ...
        for ticker in analyzed_tickers:
            ...
            # v4.14.6.93 (Fix 2): the 30-min time window is the OUTER bound
            # (don't re-fire faster than that); when change-dedup is on we ALSO
            # require (a) drift ≥ the 2% materiality floor and (b) movement ≥2%
            # since the ticker was LAST ANALYZED — so an unchanged ticker no
            # longer re-fires just because the window elapsed.
            if not _within_kind_dedup(
                    conn, ticker, path, 'price_drift', now_ts):
                drift = (current / baseline) - 1.0  # signed
                if abs(drift) >= _eff_threshold and (
                        not _pd_change_dedup
                        or _moved_since_last_fire(
                            conn, ticker, path, 'price_drift',
                            current, _PRICE_DRIFT_RECHECK_PCT)):
                    fires.append((ticker, {
                        'kind': 'price_drift',
                        'baseline': baseline,
                        'current': current,
                        'drift_pct': drift * 100.0,
                        'threshold_pct': _eff_threshold * 100.0,
                        'baseline_prediction_ts': pred.get('timestamp'),
                    }))
