...
def _load_path_config() -> dict:
    ...
    try:
        ...
        candidates = [
            __import__('tm_paths').get_data_dir() / 'path_config.json',
            __import__('tm_paths').get_app_asset_dir() / 'path_config.json',
            _P39('data') / 'path_config.json',
        ]
        try:
            here = _P39(__file__).parent
            candidates.append(__import__('tm_paths').get_app_asset_dir() / 'path_config.json')
            candidates.append(__import__('tm_paths').get_app_asset_dir() / 'path_config.json')
        except Exception:
            pass
        for p in candidates:
...
def _score_c(p: dict, live_price_fn) -> float:
    """v4.14.6.90-formula-c-sort: remaining risk-adjusted upside for one pick.

        remaining_upside = (target - live) / live      # fraction still to capture
        score_C          = remaining_upside * reward_to_risk

    Pure arithmetic — reuses the pick's already-computed reward_to_risk
    (no recompute) and a CACHE-ONLY live price from live_price_fn (the app
    passes a peek_quote/daily-bars lookup that never triggers a network fetch).

    - target ≤ live (no upside left / past target) → negative score → sinks
      (correct: played-out picks fall to the bottom).
    - live unknown (not in cache) or any bad data → 0.0 NEUTRAL, so the pick is
      ordered by the existing tiebreakers rather than being unfairly buried.
    Never raises."""
    try:
        target = float(p.get('target') or 0)
        rr = float(p.get('reward_to_risk') or 0)
        if target <= 0 or rr <= 0:
            return 0.0
        live = live_price_fn(p.get('ticker')) if live_price_fn else None
        live = float(live) if live is not None else 0.0
        if live <= 0:
            return 0.0          # unknown price → neutral (tiebreakers order it)
        remaining_upside = (target - live) / live
        return remaining_upside * rr
    except Exception:
        return 0.0
...
def filter_eligible_predictions(predictions: list[dict],
                                  ...
                                  post_filter=None,
                                  prefilter_count_out=None,
                                  live_price_fn=None) -> list[dict]:
    ...
    #
    # v4.14.6.90-formula-c-sort: when live_price_fn is supplied (the app passes a
    # CACHE-ONLY price lookup only while the rate-sort flag is ON), the PRIMARY
    # key becomes Formula C — remaining_upside × reward_to_risk — so the best
    # remaining opportunities rise and played-out picks sink. Conviction /
    # agreement / score stay as TIEBREAKERS (conviction is sparse on a fresh
    # board, so it's a tiebreak, not a gate). live_price_fn is None => the exact
    # prior ordering (byte-for-byte) — the flag-off / legacy path is unchanged.
    if live_price_fn is not None:
        # Pre-compute each pick's score once (one cache-only price read apiece)
        # so the sort key is cheap and the read happens exactly once per pick.
        _score_c_cache = {}
        for _p in deduped:
            _score_c_cache[id(_p)] = _score_c(_p, live_price_fn)

        def _sort_key(p):
            return (
                -_score_c_cache.get(id(p), 0.0),  # Formula C primary (desc)
                -p['conviction_score'],            # tiebreak: conviction
                -p['model_agreement'],             # tiebreak: agreement
                -p['score'],                       # tiebreak: R/R × conviction
            )
    else:
        def _sort_key(p):
            return (
