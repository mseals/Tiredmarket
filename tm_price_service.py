...
def _market_phase() -> str:
    ...
    v4.14.6.97-market-holiday-calendar: route through the rule-based, holiday-
    aware tm_market_calendar when enabled (so a holiday → 'closed' cadence
    1800s, not 60s over-polling of a frozen market). Falls back to the prior
    naive weekday+clock check on any error / when the flag is off."""
    try:
        import tm_market_calendar as _mc
        if _mc.is_enabled():
            return _mc.market_phase()
    except Exception:
        pass
    et = _et_now()
    if et.weekday() >= 5:
